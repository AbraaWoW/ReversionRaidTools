...
function RRT:SetupTimelineHooks(timeline)
...
function RRT:GetBossAbilityLines(encounterID, displayMode, requestedDifficulty)
...
function RRT:GetMyTimelineData(includeBossAbilities, bossDisplayMode)
    ...
    local activeReminder = RRTDB.ActiveReminder
    local reminderSource = RRTDB.Reminders
    if not activeReminder or activeReminder == "" then
        activeReminder = RRTDB.ActivePersonalReminder
        reminderSource = RRTDB.PersonalReminders
    end
    ...
    for phase, reminders in pairs(self.ProcessedReminder[encID]) do
        ...
        for _, reminder in ipairs(reminders) do
            ...
            local dur = reminder.dur or (spellID and RRTDB.ReminderSettings.SpellDuration or RRTDB.ReminderSettings.TextDuration)
...
-- Returns data in DetailsFramework timeline format
-- bossDisplayMode: "all", "important", or "combined" (see BossDisplayModes)
function RRT:GetAllTimelineData(reminderName, personal, includeBossAbilities, bossDisplayMode)
    local source = personal and RRTDB.PersonalReminders or RRTDB.Reminders
    ...
    for line in reminderStr:gmatch('[^\r\n]+') do
        ...
        if tag and time then
            ...
            if dur then
                ...
                dur = spellID and RRTDB.ReminderSettings.SpellDuration or RRTDB.ReminderSettings.TextDuration
            end
    ...
    for _, player in ipairs(sortedPlayers) do
        ...
        for _, ability in ipairs(sortedAbilities) do
            ...
            local unitName = RRTAPI:GetChar(player, true, "ReversionRaidTools")
...
function RRT:CreateTimelineWindow()
    ...
    local timelineWindow = DF:CreateSimplePanel(UIParent, window_width, window_height,
        "|cFF00FFFFReversion Raid|r Timeline", "RRTUITimelineWindow", {
        DontRightClickClose = true,
        UseStatusBar = false,
            UseScaleBar = true,
        },
        RRTDB.RRTUI.timeline_window)
    ...
    timelineWindow.bossDisplayMode = RRT.BossDisplayModes.IMPORTANT_HEALER -- Default display mode
        ...
        return {
            {
                ...
                value = RRT.BossDisplayModes.IMPORTANT_HEALER,
                onclick = function(_, _, value)
            ...
            },
            {
                ...
                value = RRT.BossDisplayModes.IMPORTANT_TANK,
                onclick = function(_, _, value)
                    timelineWindow.bossDisplayMode = value
                    RRT:RefreshTimelineForMode()
                end
            ...
            {
                label = "Show All",
                value = RRT.BossDisplayModes.SHOW_ALL,
                onclick = function(_, _, value)
                    timelineWindow.bossDisplayMode = value
                    RRT:RefreshTimelineForMode()
                end
            ...
            {
                label = "Combined",
                value = RRT.BossDisplayModes.COMBINED,
                onclick = function(_, _, value)
                    timelineWindow.bossDisplayMode = value
                    RRT:RefreshTimelineForMode()
                end
            },
            {
                ...
                value = RRT.BossDisplayModes.COMBINED_IMPORTANT,
                onclick = function(_, _, value)
                    timelineWindow.bossDisplayMode = value
                    RRT:RefreshTimelineForMode()
                end
    ...
    local bossDisplayDropdown = DF:CreateDropDown(timelineWindow, BuildBossDisplayModeOptions, RRT.BossDisplayModes.IMPORTANT_HEALER, 150)
...
function RRT:ToggleTimelineWindow()
...
function RRT:RefreshTimelineForMode()
...
function RRT:AutoFitTimelineScale(timeline, dataLength)
    if not timeline then return end
...
function RRT:RefreshMyRemindersTimeline()
...
function RRT:RefreshAllRemindersTimeline(reminderName, personal)
    if not self.TimelineWindow or not self.TimelineWindow.timeline then return end
...
function RRT:UpdateTimelineTitle()
    local window = self.TimelineWindow
    if not window then return end

    local title = "|cFF00FFFFReversion Raid|r Timeline"
    ...
    if encID then
        ...
        if difficulty then
            title = string.format("|cFF00FFFFReversion Raid|r Timeline - %s (%s)", bossName, difficulty)
        else
            title = string.format("|cFF00FFFFReversion Raid|r Timeline - %s", bossName)
        end
    end
...
function RRT:UpdatePhaseMarkers()
    local window = self.TimelineWindow
    if not window then return end
        ...
        if phaseNum > 1 then
            ...
            if not marker then
                ...
                marker:SetScript("OnDragStop", function(self)
                    ...
                    RRT:SetPhaseStart(self.encID, self.phaseNum, newTime)
                ...
                marker:SetScript("OnEnter", function(self)
                    ...
                    local time = RRT:GetPhaseStart(self.encID, self.phaseNum)
                ...
                marker:SetScript("OnMouseDown", function(self, button)
                    if button == "RightButton" then
                        RRT:ResetPhaseStart(self.encID, self.phaseNum)
                        RRT:RefreshTimelineForMode()
                    end
                end)
...
function RRT:RefreshEmbeddedTimeline(tab)
...
function RRT:RefreshEmbeddedMyReminders(tab)
    if not tab or not tab.timeline then return end
...
function RRT:RefreshEmbeddedAllReminders(tab, reminderName, personal)
    if not tab or not tab.timeline then return end
...
function RRT:UpdateEmbeddedTimelineTitle(tab)
...
function RRT:UpdateEmbeddedPhaseMarkers(tab)
        ...
        if phaseNum > 1 then
            ...
            if not marker then
                ...
                marker:SetScript("OnDragStop", function(self)
                    ...
                    RRT:SetPhaseStart(self.encID, self.phaseNum, newTime)
                    RRT:RefreshEmbeddedTimeline(self.parentTab)
                end)
