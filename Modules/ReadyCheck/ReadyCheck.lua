...
function RRT_NS:SoulstoneCheck()
...
function RRT_NS:GemCheck(slot, itemString)
    local gemsMissing = 0
...
function RRT_NS:EnchantCheck(slot, itemString)
...
function RRT_NS:GearCheck()
    ...
    for slot = 1, #SlotName do
        ...
        if itemString then
            if RRT.ReadyCheckSettings.CraftedCheck and string.find(itemString, "8960") then
                crafted = crafted+1
            end
            if RRT.ReadyCheckSettings.EnchantCheck and self:EnchantCheck(slot,itemString) then
                table.insert(missing, "Missing Enchant on: |cFF00FF00"..SlotName[slot].."|r")
            end
            if RRT.ReadyCheckSettings.GemCheck and self:GemCheck(slot, itemString) then
                table.insert(missing, "Missing Gem in: |cFF00FF00"..SlotName[slot].."|r")
            end
            if RRT.ReadyCheckSettings.ItemLevelCheck and slot ~= 4 and select(4, C_Item.GetItemInfo(itemString)) < ilvl then
                table.insert(missing, "Low Itemlvl equipped on: |cFF00FF00"..SlotName[slot].."|r")
            end
            if RRT.ReadyCheckSettings.RepairCheck and not repair then
                local min, max = GetInventoryItemDurability(slot)
                if min and min/max <= 0.2 then
                    repair = true
                end
            end
            if RRT.ReadyCheckSettings.TierCheck then
            ...
            if MyArmorType ~= 1 and RRT.ReadyCheckSettings.MissingItemCheck and slot ~= 4 and slot ~= 15 and slot ~= 16 and slot ~= 17 then
        ...
        elseif RRT.ReadyCheckSettings.MissingItemCheck and slot ~= 4 then
            if slot == 17 then
...
function RRT_NS:GatewayControlCheck()
    for bagID = 0, NUM_BAG_SLOTS do
        for invID = 1, C_Container.GetContainerNumSlots(bagID) do
            ...
            if itemID and itemID == 188152 then -- Gateway Shard found in Inventory
                ...
                bound = bound or (RRT.ReadyCheckSettings.SkipGatewayKeybindCheck and onbar)
                if bound then
                    return false
                elseif onbar then
...
function RRT_NS:CheckGateWayKeybind(Slot)
...
function RRT_NS:TierCheck(Slot)
    local tier = 0
