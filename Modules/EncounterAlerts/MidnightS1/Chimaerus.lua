...
local function RiftMadnessTimers(id)
    ...
    if diff == 16 and RRT.EncounterAlerts[encID].enabled then -- text, Type, spellID, dur, phase, encID
        if UnitGroupRolesAssigned("player") == "TANK" then return end
        local dur = 6
        local Alert = RRT_NS:CreateDefaultAlert("Debuffs Soon", "Text", nil, dur, 1, encID) -- Group Soaks
        Alert.TTS = false
        local timers = {39, 112}
        if RRT_NS.AlertTimers then
            for i, v in ipairs(RRT_NS.AlertTimers) do
                if v and v.Cancel then
                    v:Cancel()
                end
            end
            RRT_NS.AlertTimers = nil
        end
        RRT_NS.AlertTimers = {}
        for i, v in ipairs(timers or {}) do
            RRT_NS.AlertTimers[i] = C_Timer.NewTimer(v-dur, function()
                if UnitExists("boss2") then
                    RRT_NS:DisplayReminder(Alert)
                end
            end)
        end
    end
...
RRT_NS.EncounterAlertStart[encID] = function(self, id) -- on ENCOUNTER_START
    ...
    if RRT.EncounterAlerts[encID].enabled then
