...
RRT_NS.EncounterAlertStart[encID] = function(self, id) -- on ENCOUNTER_START
    ...
    if RRT.EncounterAlerts[encID].enabled or encID == 3463 then -- text, Type, spellID, dur, phase, encID
        --[[
...
RRT_NS.ShowWarningAlert[encID] = function(self, encID, phase, time, info) -- on ENCOUNTER_WARNING
...
RRT_NS.ShowBossWhisperAlert[encID] = function(self, encID, phase, time, text, name, dur) -- on RAID_BOSS_WHISPER
    if RRT.EncounterAlerts[encID].enabled then
