...
RRT_NS.Groups = {}
RRT_NS.Groups.Processing = false

RRT_NS.meleetable = { -- ignoring tanks for this
...
function RRT_NS:GetSortedGroup(Flex, default, odds)
    local units = {}
...
function RRT_NS:SortGroup(Flex, default, odds) -- default == tank, melee, ranged, healer
    self.Groups.units = self:GetSortedGroup(Flex, default, odds)
    self:ArrangeGroups(true)
end

function RRT_NS:ShiftLeader(group)
...
function RRT_NS:ArrangeGroups(firstcall, finalcheck)
...
function RRT_NS:SplitGroupInit(Flex, default, odds)
    ...
    if UnitIsGroupAssistant("player") or UnitIsGroupLeader("player") and UnitInRaid("player") then
        ...
        if not self.LastGroupSort or self.LastGroupSort < now - 5 then
            ...
            self.specs = {}
            self:Broadcast("RRT_SPEC_REQUEST", "RAID", "nilcheck")
...
RRT_NS.RaidBuffCheck = DF:CreateSimplePanel(UIParent, 300, 300, "[RRT] Missing Raid Buffs",
        "NSIRaidBuffFrame", {
        })
RRT_NS.RaidBuffCheck:Hide()
local MissingBuffLabel = DF:CreateLabel(RRT_NS.RaidBuffCheck, "", 14)
MissingBuffLabel:SetPoint("TOPLEFT", RRT_NS.RaidBuffCheck, "TOPLEFT", 2, -60)
...
function RRT_NS:UpdateRaidBuffFrame()
    if not RRT.Settings.MissingRaidBuffs or not UnitInRaid("player") or not (UnitIsGroupLeader("player") or UnitIsGroupAssistant("player") or RRT.Settings.Debug) then
    ...
    if PVEFrame:IsShown() and PanelTemplates_GetSelectedTab(PVEFrame) == nil then -- first time opening PVE frame, tab info is not yet available
        C_Timer.After(0.1, function() RRT_NS:UpdateRaidBuffFrame() end)
        return
    end
    ...
    if parent then
        ...
        local maxgroup = RRT.Settings.FlexRaid and 6 or 4
...
FriendsFrame:HookScript("OnShow", function() RRT_NS:UpdateRaidBuffFrame() end)
FriendsFrame:HookScript("OnHide", function() RRT_NS:UpdateRaidBuffFrame() end)
PVEFrame:HookScript("OnShow", function() RRT_NS:UpdateRaidBuffFrame() end)
PVEFrame:HookScript("OnHide", function() RRT_NS:UpdateRaidBuffFrame() end)
PVEFrameTab1:HookScript("OnClick", function() RRT_NS:UpdateRaidBuffFrame() end)
PVEFrameTab2:HookScript("OnClick", function() RRT_NS:UpdateRaidBuffFrame() end)
PVEFrameTab3:HookScript("OnClick", function() RRT_NS:UpdateRaidBuffFrame() end)
FriendsFrameTab1:HookScript("OnClick", function() RRT_NS:UpdateRaidBuffFrame() end)
FriendsFrameTab2:HookScript("OnClick", function() RRT_NS:UpdateRaidBuffFrame() end)
FriendsFrameTab3:HookScript("OnClick", function() RRT_NS:UpdateRaidBuffFrame() end)
FriendsFrameTab4:HookScript("OnClick", function() RRT_NS:UpdateRaidBuffFrame() end)

function RRT_NS:InviteFromReminder(str, init)
    local list = RRT.InviteList[str]
...
function RRT_NS:InviteList(list)
...
function RRT_NS:ArrangeFromReminder(str)
