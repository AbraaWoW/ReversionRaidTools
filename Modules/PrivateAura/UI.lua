...
local function BuildPASoundEditUI()
    ...
    function RRT_NS:RefreshPASoundEditUI()
    ...
    local function createLineFunc(self, index)
        ...
        line.sounddropdown:SetHook("OnOptionSelected", function(self, _, value)
            ...
            RRT_NS:SavePASound(tonumber(line.spellID), newValue)
        ...
        line.deleteButton = DF:CreateButton(line, function()
            RRT_NS:SavePASound(tonumber(line.spellID), nil)
            self:SetData(RRT.PASounds)
            self:MasterRefresh()
        end, 12, 12)
    ...
    local add_button = DF:CreateButton(PASound_edit_frame, function()
        ...
        if spellID and sound ~= "" then
            ...
            if C_UnitAuras.AuraIsPrivate(spellID) then
                RRT_NS:SavePASound(spellID, sound)
            else
    ...
    local function DeleteAllPASounds(self)
        local popup = DF:CreateSimplePanel(UIParent, 300, 150, "Confirm Deleting ALL Private Aura Sounds", "RRTDeleteALLPASoundsPopup")
        ...
        local confirmButton = DF:CreateButton(popup, function()
            for spellID, info in pairs(RRT.PASounds) do
                if info and type(info) == "table" and info.sound then
                    RRT_NS:AddPASound(spellID, nil)
                end
            end
            RRT.PASounds = {
                UseDefaultPASounds = RRT.PASounds.UseDefaultPASounds,
                UseDefaultMPlusPASounds = RRT.PASounds.UseDefaultMPlusPASounds
            }
            PASound_edit_scrollbox:MasterRefresh()
            popup:Hide()
        end, 100, 20, "Confirm")
...
local function BuildBuffSoundEditUI()
    local frame = DF:CreateSimplePanel(UIParent, 485, 420, "Buff Sounds", "BuffSoundEditFrame", {
        DontRightClickClose = true
    })
    ...
    local function PrepareData()
        ...
        for spellID, soundName in pairs(RRT.BuffSounds or {}) do
            ...
            if sid and soundName then
                local spell = C_Spell.GetSpellInfo(sid)
                tinsert(data, { sound = soundName, spellID = sid, name = spell and spell.name or tostring(sid) })
            end
        end
    ...
    function RRT_NS:RefreshBuffSoundEditUI()
        frame.scrollbox:MasterRefresh()
    end
    ...
    local function refresh(self, data, offset, totalLines)
        for i = 1, totalLines do
            ...
            if Data and Data.sound then
                ...
                line.spellIDText.text = Data.spellID
                line.sound            = Data.sound
                line.texture          = C_Spell.GetSpellTexture(Data.spellID)
                line.sounddropdown:Select(line.sound)
                line.spellIcon:SetTexture(line.texture)
            end
        end
    ...
    local function createLineFunc(self, index)
        ...
        line.spellIcon:SetScript("OnLeave", function() GameTooltip:Hide() end)
        ...
        line.sounddropdown = DF:CreateDropDown(line, function() return build_sound_dropdown() end, nil, 170)
        line.sounddropdown:SetTemplate(options_dropdown_template)
        line.sounddropdown:SetPoint("LEFT", line.spellIDText, "RIGHT", 5, 0)
        line.sounddropdown:SetHook("OnOptionSelected", function(_, _, value)
            local newValue = soundlist[value]
            if newValue == line.sound then return end
            RRT_NS:SaveBuffSound(tonumber(line.spellID), newValue)
            line.sound = newValue
            self:MasterRefresh()
        end)

        line.deleteButton = DF:CreateButton(line, function()
            RRT_NS:DeleteBuffSound(tonumber(line.spellID))
            self:MasterRefresh()
        end, 12, 12)
        line.deleteButton:SetNormalTexture([[Interface\GLUES\LOGIN\Glues-CheckBox-Check]])
        line.deleteButton:SetHighlightTexture([[Interface\GLUES\LOGIN\Glues-CheckBox-Check]])
    ...
    local scrollLines = 15
    local scrollbox = DF:CreateScrollBox(frame, "$parentBuffSoundsEditScrollBox", refresh,
        {}, 445, 300, scrollLines, 20, createLineFunc)
    frame.scrollbox = scrollbox
    scrollbox:SetPoint("TOPLEFT", frame, "TOPLEFT", 10, -50)
    ...
    local SpellName = DF:CreateLabel(frame, "Spell Name", 11)
    SpellName:SetPoint("TOPLEFT", frame, "TOPLEFT", 40, -30)
    SpellName:SetWidth(100)
    local SpellID = DF:CreateLabel(frame, "Spell-ID", 11)
    SpellID:SetPoint("LEFT", SpellName, "RIGHT", 55, 0)
    SpellID:SetWidth(70)
    local Sound = DF:CreateLabel(frame, "Sound", 11)
    Sound:SetWidth(120)
    Sound:SetPoint("LEFT", SpellID, "RIGHT", 0, 0)

    scrollbox:SetScript("OnShow", function(self) self:MasterRefresh() end)

    local NewSpellID = DF:CreateLabel(frame, "SpellID:", 11)
    NewSpellID:SetPoint("BOTTOMLEFT", frame, "BOTTOMLEFT", 10, 50)
    NewSpellID:SetWidth(80)

    local NewSpellIDTextEntry = DF:CreateTextEntry(frame, function() end, 120, 20)
    NewSpellIDTextEntry:SetPoint("LEFT", NewSpellID, "RIGHT", -10, 0)
    NewSpellIDTextEntry:SetTemplate(options_dropdown_template)

    local NewSound = DF:CreateLabel(frame, "Sound:", 11)
    NewSound:SetPoint("LEFT", NewSpellIDTextEntry, "RIGHT", 10, 0)
    NewSound:SetWidth(80)

    local NewSoundDropdown = DF:CreateDropDown(frame, function() return build_sound_dropdown() end, nil, 120)
    NewSoundDropdown:SetPoint("LEFT", NewSound, "RIGHT", -10, 0)
    NewSoundDropdown:SetTemplate(options_dropdown_template)

    local add_button = DF:CreateButton(frame, function()
        local spellID = tonumber(NewSpellIDTextEntry:GetText())
        local sound   = soundlist[NewSoundDropdown:GetValue()]
        if spellID and sound and sound ~= "" then
            RRT_NS:SaveBuffSound(spellID, sound)
            NewSpellIDTextEntry:SetText("")
            NewSoundDropdown:SetValue(nil)
...
local function BuildDebuffSoundEditUI()
    local frame = DF:CreateSimplePanel(UIParent, 485, 420, "Debuff Sounds", "DebuffSoundEditFrame", {
        DontRightClickClose = true
    })
    frame:SetPoint("CENTER", UIParent, "CENTER", 0, 0)
    ...
    function RRT_NS:RefreshDebuffSoundEditUI()
        frame.scrollbox:MasterRefresh()
    end
    ...
    local function createLineFunc(self, index)
        ...
        line.sounddropdown:SetHook("OnOptionSelected", function(_, _, value)
            local newValue = soundlist[value]
            if newValue == line.sound then return end
            RRT_NS:SaveDebuffSound(tonumber(line.spellID), newValue)
            line.sound = newValue
            self:MasterRefresh()
        end)

        line.deleteButton = DF:CreateButton(line, function()
            RRT_NS:DeleteDebuffSound(tonumber(line.spellID))
            self:MasterRefresh()
        end, 12, 12)
        line.deleteButton:SetNormalTexture([[Interface\GLUES\LOGIN\Glues-CheckBox-Check]])
        line.deleteButton:SetHighlightTexture([[Interface\GLUES\LOGIN\Glues-CheckBox-Check]])
    ...
    local scrollLines = 15
    local scrollbox = DF:CreateScrollBox(frame, "$parentDebuffSoundsEditScrollBox", refresh,
        {}, 445, 300, scrollLines, 20, createLineFunc)
    frame.scrollbox = scrollbox
    scrollbox:SetPoint("TOPLEFT", frame, "TOPLEFT", 10, -50)
    scrollbox.MasterRefresh = MasterRefresh
    ...
    local add_button = DF:CreateButton(frame, function()
        ...
        local sound   = soundlist[NewSoundDropdown:GetValue()]
        if spellID and sound and sound ~= "" then
            RRT_NS:SaveDebuffSound(spellID, sound)
            NewSpellIDTextEntry:SetText("")
            NewSoundDropdown:SetValue(nil)
            scrollbox:MasterRefresh()
        end
...
RRT_NS.UI.PrivateAuras = {
    BuildPASoundEditUI     = BuildPASoundEditUI,
    BuildBuffSoundEditUI   = BuildBuffSoundEditUI,
    BuildDebuffSoundEditUI = BuildDebuffSoundEditUI,
}
