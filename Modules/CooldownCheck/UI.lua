...
local function build_spec_options()
    local t = {}
    local classSpecs = RRT_NS.CLASS_SPECIALIZATION_MAP

    for className, specs in pairs(classSpecs) do
        for _, specId in pairs(specs) do
            tinsert(t, {
                label = RRT_NS:SpecToName(specId),
                value = specId,
                className = className,
            })
        end
...
local function BuildCooldownsEditUI()
    ...
    local function PrepareData(data)
        local data = {}
        for specId, cooldowns in pairs(RRT.CooldownList) do
    ...
    local function refresh(self, data, offset, totalLines)
        for i = 1, totalLines do
            ...
            if cooldownData then
                ...
                line.specText.text = RRT_NS:SpecToName(line.spec)
    ...
    local function createLineFunc(self, index)
        ...
        line.typeDropdown:SetHook("OnOptionSelected", function(self, _, value)
            ...
            if newType == "Spell" then
                RRT_NS:RemoveTrackedCooldown(line.spec, line.id, string.lower(oldType))
                RRT_NS:AddTrackedCooldown(line.spec, line.id, "spell", line.offset)
            else
                RRT_NS:RemoveTrackedCooldown(line.spec, line.id, string.lower(oldType))
                RRT_NS:AddTrackedCooldown(line.spec, line.id, "item", line.offset)
            end
        ...
        line.idTextEntry:SetScript("OnEnterPressed", function(self)
            ...
            if line.type == "Spell" then
                RRT_NS:RemoveTrackedCooldown(line.spec, oldId, "spell")
                RRT_NS:AddTrackedCooldown(line.spec, newId, "spell", line.offset)
            else
                RRT_NS:RemoveTrackedCooldown(line.spec, oldId, "item")
                RRT_NS:AddTrackedCooldown(line.spec, newId, "item", line.offset)
            end
        ...
        line.offsetSlider:SetHook("OnValueChanged", function(self, fixedValue, value)
            RRT_NS:RemoveTrackedCooldown(line.spec, line.id, line.type)
            RRT_NS:AddTrackedCooldown(line.spec, line.id, line.type, value)
            line.offset = value
            parent:MasterRefresh()
        end)
    ...
    local add_button = DF:CreateButton(cooldowns_edit_frame, function()
        ...
        if spec and id ~= "" then
            RRT_NS:AddTrackedCooldown(spec, id, type, offset)
