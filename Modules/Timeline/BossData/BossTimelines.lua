...
--[[
    ...
    RRT_NS.BossTimelines[encounterID] = {
...
RRT_NS.BossTimelines = RRT_NS.BossTimelines or {}
...
RRT_NS.BossTimelineCategoryOrder = {
    ["raid damage"] = 1,
    ["raid aoe"] = 2,
    ["raid soak"] = 3,
    ["group soak"] = 4,
...
RRT_NS.BossTimelineImportantHealerCategories = {
    ["raid damage"] = true,
    ["raid aoe"] = true,
    ["raid soak"] = true,
    ["group soak"] = true,
...
RRT_NS.BossTimelineImportantTankCategories = {
    ["tankbuster"] = true,
    ["tank debuff"] = true,
    ["frontal"] = true,
}
...
function RRT_NS:IsAbilityImportantForHealer(ability)
...
function RRT_NS:IsAbilityImportantForTank(ability)
    -- Explicit important flag takes precedence
    if ability.important ~= nil then
        return ability.important
    end
...
function RRT_NS:IsAbilityImportant(ability)
...
function RRT_NS:ParseCategoryForDisplay(categoryStr)
    if not categoryStr or categoryStr == "" then
...
RRT_NS.DifficultyNames = {
    [15] = "Heroic",
    [16] = "Mythic",
}

-- Get current difficulty name, defaults to "Mythic" if unknown
function RRT_NS:GetCurrentDifficultyName()
...
function RRT_NS:GetBossTimeline(encounterID, difficulty)
...
function RRT_NS:GetPhaseStart(encounterID, phaseNum, difficulty)
    ...
    if RRT.PhaseTimings and RRT.PhaseTimings[encounterID] and RRT.PhaseTimings[encounterID][phaseNum] then
        return RRT.PhaseTimings[encounterID][phaseNum]
    end
...
function RRT_NS:SetPhaseStart(encounterID, phaseNum, time)
    ...
    if not RRT.PhaseTimings then
        RRT.PhaseTimings = {}
    end
    if not RRT.PhaseTimings[encounterID] then
        RRT.PhaseTimings[encounterID] = {}
    end
    ...
    RRT.PhaseTimings[encounterID][phaseNum] = time
        ...
        if timeline and timeline.phases then
            for otherPhaseNum, _ in pairs(timeline.phases) do
                if otherPhaseNum > phaseNum then
                    ...
                    RRT.PhaseTimings[encounterID][otherPhaseNum] = newOtherTime
                end
            end
        end
    end
...
function RRT_NS:ResetPhaseStart(encounterID, phaseNum)
    if RRT.PhaseTimings and RRT.PhaseTimings[encounterID] then
        RRT.PhaseTimings[encounterID][phaseNum] = nil
    end
end

-- Get all abilities for an encounter with absolute times
function RRT_NS:GetBossTimelineAbilities(encounterID, difficulty)
...
function RRT_NS:GetEncounterName(encounterID)
