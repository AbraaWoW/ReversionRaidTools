...
local function BuildTimelineTabUI(parent)
    ...
    parent.bossDisplayMode = RRT_NS.BossDisplayModes.IMPORTANT_HEALER
        ...
        return {
            {
                ...
                onclick = function(_, _, value)
                    ...
                    RRT_NS:RefreshEmbeddedTimeline(parent)
                end
            },
            {
    ...
    local function BuildReminderDropdownOptions()
        local options = {}
        local sharedList = RRT_NS:GetAllReminderNames(false)
        ...
        local personalList = RRT_NS:GetAllReminderNames(true)
    ...
    local bossDisplayDropdown = DF:CreateDropDown(parent, BuildBossDisplayModeOptions, RRT_NS.BossDisplayModes.IMPORTANT_HEALER, 150)
    ...
    local noDataLabel = DF:CreateLabel(parent, "No reminders to display. Load a reminder set first with /rvr", 14, "gray")
    ...
    RRT_NS:SetupTimelineHooks(timelineFrame)
    ...
    if timelineFrame.scaleSlider then
        timelineFrame.scaleSlider:HookScript("OnValueChanged", function()
            RRT_NS:UpdateEmbeddedPhaseMarkers(parent)
        end)
    ...
    parent:SetScript("OnShow", function(self)
        RRT_NS:RefreshEmbeddedTimeline(self)
    end)
