...
local function ExportSettings()
    ...
    for key in pairs(defaults) do
        if db[key] ~= nil then
            export[key] = DeepCopy(db[key])
        end
    end

    -- Also include custom buffs
    if db.customBuffs then
        export.customBuffs = DeepCopy(db.customBuffs)
    end
...
local function ImportSettings(str)
    ...
    -- Deep merge imported data into RRTDB.BuffReminders
...
-- ============================================================================
-- Ensure the BuffReminders global exists for external addon compatibility.
BuffReminders = BuffReminders or {}
