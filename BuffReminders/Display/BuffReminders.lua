...
local function GetDB()
    return RRTDB and RRTDB.BuffReminders
end
...
local masqueGroup = Masque and Masque:Group("ReversionRaidTools", "Buff Reminders")
...
local function ResolveFontPath()
    local db = GetDB()
    local fontName = db and db.defaults and db.defaults.fontFace
    if fontName then
...
local function ShouldShowText(category)
    ...
    local db = GetDB()
    local cs = db and db.categorySettings and db.categorySettings[category]
...
local function ResolveConsumableFrame(frame)
    ...
    if (GetDB() and GetDB().defaults or {}).showConsumablesWithoutItems then
        return "missing"
    end
    return false
end
...
local function ApplyConsumableDisplayMode(frame, entry, frameList, parentFrame)
    ...
    local _db = GetDB()
    local displayMode = (_db and _db.defaults or {}).consumableDisplayMode or "sub_icons"
    local items = frame._cachedItems
    if items == nil then
        items = BR.SecureButtons.GetConsumableActionItems(frame.buffDef) or false
        frame._cachedItems = items
    ...
    if displayMode == "sub_icons" then
        local cs = _db and _db.categorySettings and _db.categorySettings.consumable
...
local function ApplyPetDisplayMode(frame, entry, frameList)
    ...
    local petMode = (GetDB() and GetDB().defaults or {}).petDisplayMode or "generic"
...
UpdateDisplay = function()
    ...
    if not testMode then
        ...
        if not InCombatLockdown() then
            local displayMode = (GetDB() and GetDB().defaults or {}).consumableDisplayMode
...
BR.CustomBuffs = {
    ...
    UpdateFrame = function(key, spellIDValue, displayName)
        ...
        if frame then
            ...
            local customBuff = RRTDB.BuffReminders and RRTDB.BuffReminders.customBuffs and RRTDB.BuffReminders.customBuffs[key]
...
local function UpdateVisuals()
    for _, frame in pairs(buffFrames) do
        ...
        if frame.buffText then
            local _db = GetDB()
            local raidBts = _db and _db.categorySettings
                and _db.categorySettings.raid
                and _db.categorySettings.raid.buffTextSize
            ...
            if buffCat == "raid" then
                local cs = _db and _db.categorySettings and _db.categorySettings.raid
...
local function SlashHandler(msg)
    ...
    elseif cmd == "lock" then
        RRTDB.BuffReminders.locked = true
        BR.Movers.HideAll()
        BR.Components.RefreshAll()
        print("|cff00ccffBuff Reminders:|r Frames locked.")
    elseif cmd == "unlock" then
        RRTDB.BuffReminders.locked = false
        BR.Movers.UpdateAnchor()
        BR.Components.RefreshAll()
        print("|cff00ccffBuff Reminders:|r Frames unlocked.")
    elseif cmd == "rrt" then
        -- Open the RRT UI on the BuffReminders tab
        ...
        -- Open the full Buff Reminders options panel (/br) or open RRT UI (/br rrt)
        BR.Options.Toggle()
    end
end
...
eventFrame:SetScript("OnEvent", function(_, event, arg1, arg2)
    if event == "ADDON_LOADED" and arg1 == addonName then
        ...
        if not RRTDB.BuffReminders then
            RRTDB.BuffReminders = {}
        end
        ...
        SLASH_BUFFREMINDERS1 = "/buffreminders"
        SlashCmdList["BUFFREMINDERS"] = SlashHandler

        -- Register with WoW's Interface Options (integrated into Reversion Raid Tools)
        local settingsPanel = CreateFrame("Frame")
        settingsPanel.name = "BuffReminders"
        settingsPanel.parent = "Reversion Raid Tools"

        local title = settingsPanel:CreateFontString(nil, "ARTWORK", "GameFontNormalLarge")
        title:SetPoint("TOPLEFT", 16, -16)
        title:SetText("|cFF00FFFFBuff Reminders|r — Reversion Raid Tools")

        local desc = settingsPanel:CreateFontString(nil, "ARTWORK", "GameFontHighlight")
        desc:SetPoint("TOPLEFT", title, "BOTTOMLEFT", 0, -8)
        desc:SetText("Buff Reminders is integrated into Reversion Raid Tools.")
        ...
        slashInfo:SetText("Use |cFFFFFF00/rrt|r to open options")
    ...
    elseif event == "PLAYER_ENTERING_WORLD" then
        ...
        if not mainFrame then
            ...
            for _, cat in ipairs(CATEGORIES) do
                local cs = RRTDB.BuffReminders.categorySettings and RRTDB.BuffReminders.categorySettings[cat]
        ...
        C_Timer.After(1.5, function()
            for key, def in pairs(RRTDB.BuffReminders.customBuffs or {}) do
                local frame = buffFrames[key]
    ...
    elseif event == "READY_CHECK" then
        ...
        local duration = RRTDB.BuffReminders.readyCheckDuration or 15
