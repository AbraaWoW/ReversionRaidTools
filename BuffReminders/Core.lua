...
if type(CreateFromMixins) == "function" and type(CallbackRegistryMixin) == "table" then
    CallbackRegistry = CreateFromMixins(CallbackRegistryMixin)
    ...
    -- Minimal callback bus fallback when Blizzard mixin is unavailable.
    local listeners = {}
    CallbackRegistry = {}
    function CallbackRegistry:RegisterCallback(eventName, func)
    ...
    function CallbackRegistry:TriggerEvent(eventName, ...)
        local funcs = listeners[eventName]
        if not funcs then
            return
        end
        for i = 1, #funcs do
...
local GetSpecInfoForClassID = GetSpecializationInfoForClassID
if type(GetSpecInfoForClassID) ~= "function"
    and C_SpecializationInfo
    and type(C_SpecializationInfo.GetSpecializationInfoForClassID) == "function" then
    GetSpecInfoForClassID = C_SpecializationInfo.GetSpecializationInfoForClassID
end
for token, classID in pairs(CLASS_IDS) do
    local specs = {}
    if type(GetSpecInfoForClassID) == "function" then
        for i = 1, 4 do
            local specID, name = GetSpecInfoForClassID(classID, i)
            if specID then
                table.insert(specs, { value = specID, label = name or tostring(specID) })
            end
        end
    end
