...
RRT.BossTimelines = RRT.BossTimelines or {}
...
RRT.BossTimelineImportantHealerCategories = {
    ["raid damage"] = true,
    ["raid aoe"] = true,
    ["raid soak"] = true,
    ["group soak"] = true,
...
RRT.BossTimelineImportantTankCategories = {
    ["tankbuster"] = true,
    ["tank debuff"] = true,
    ["frontal"] = true,
}
...
function RRT:IsAbilityImportantForHealer(ability)
...
function RRT:IsAbilityImportantForTank(ability)
    -- Explicit important flag takes precedence
    if ability.important ~= nil then
        return ability.important
    end
...
function RRT:IsAbilityImportant(ability)
...
function RRT:ParseCategoryForDisplay(categoryStr)
    if not categoryStr or categoryStr == "" then
...
function RRT:GetCurrentDifficultyName()
...
function RRT:GetBossTimeline(encounterID, difficulty)
...
function RRT:GetPhaseStart(encounterID, phaseNum, difficulty)
    ...
    if RRTDB.PhaseTimings and RRTDB.PhaseTimings[encounterID] and RRTDB.PhaseTimings[encounterID][phaseNum] then
        return RRTDB.PhaseTimings[encounterID][phaseNum]
    end
...
function RRT:SetPhaseStart(encounterID, phaseNum, time)
    ...
    if not RRTDB.PhaseTimings then
        RRTDB.PhaseTimings = {}
    end
    if not RRTDB.PhaseTimings[encounterID] then
        RRTDB.PhaseTimings[encounterID] = {}
    end
    ...
    RRTDB.PhaseTimings[encounterID][phaseNum] = time
        ...
        if timeline and timeline.phases then
            for otherPhaseNum, _ in pairs(timeline.phases) do
                if otherPhaseNum > phaseNum then
                    ...
                    RRTDB.PhaseTimings[encounterID][otherPhaseNum] = newOtherTime
                end
            end
        end
    end
...
function RRT:ResetPhaseStart(encounterID, phaseNum)
    if RRTDB.PhaseTimings and RRTDB.PhaseTimings[encounterID] then
        RRTDB.PhaseTimings[encounterID][phaseNum] = nil
    end
end

-- Get all abilities for an encounter with absolute times
function RRT:GetBossTimelineAbilities(encounterID, difficulty)
...
function RRT:GetEncounterName(encounterID)
