...
function RRT:Broadcast(event, channel, ...) -- using internal broadcast function for anything inside the addon to prevent users to send stuff they shouldn't be sending
    local message = event
    local argTable = {...}
    local target = ""
...
local function ReceiveComm(text, chan, sender, whisper, internal)
    ...
    if (UnitExists(sender) and (UnitInRaid(sender) or UnitInParty(sender))) or (chan == "GUILD" and allowedcomms[event]) then -- block addon msg's from outside the raid, only exception being the guild nickname comms.
        ...
        RRT:EventHandler(event, false, internal, unpack(formattedArgTable))
