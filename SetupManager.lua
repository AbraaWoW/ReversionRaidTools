...
RRT.Groups = {}
RRT.Groups.Processing = false

RRT.meleetable = { -- ignoring tanks for this
...
function RRT:GetSortedGroup(Flex, default, odds)
    local units = {}
...
function RRT:SortGroup(Flex, default, odds) -- default == tank, melee, ranged, healer
    self.Groups.units = self:GetSortedGroup(Flex, default, odds)
    self:ArrangeGroups(true)
end

function RRT:ShiftLeader(group)
...
function RRT:ArrangeGroups(firstcall, finalcheck)
...
function RRT:SplitGroupInit(Flex, default, odds)
    ...
    if UnitIsGroupAssistant("player") or UnitIsGroupLeader("player") and UnitInRaid("player") then
        ...
        if not self.LastGroupSort or self.LastGroupSort < now - 5 then
            ...
            self.specs = {}
            RRT:Broadcast("RRT_SPEC_REQUEST", "RAID", "nilcheck")
...
RRT.RaidBuffCheck = DF:CreateSimplePanel(UIParent, 300, 300, "[RRTDB] Missing Raid Buffs",
        "RRTRaidBuffFrame", {
        })
RRT.RaidBuffCheck:Hide()
local MissingBuffLabel = DF:CreateLabel(RRT.RaidBuffCheck, "", 14)
MissingBuffLabel:SetPoint("TOPLEFT", RRT.RaidBuffCheck, "TOPLEFT", 2, -60)
...
function RRT:UpdateRaidBuffFrame()
    if not RRTDB.Settings.MissingRaidBuffs or not UnitInRaid("player") or not (UnitIsGroupLeader("player") or UnitIsGroupAssistant("player") or RRTDB.Settings.Debug) then
    ...
    if PVEFrame:IsShown() and PanelTemplates_GetSelectedTab(PVEFrame) == nil then -- first time opening PVE frame, tab info is not yet available
        C_Timer.After(0.1, function() RRT:UpdateRaidBuffFrame() end)
        return
    end
    ...
    if parent then
        ...
        local maxgroup = RRTDB.Settings.FlexRaid and 6 or 4
...
FriendsFrame:HookScript("OnShow", function() RRT:UpdateRaidBuffFrame() end)
FriendsFrame:HookScript("OnHide", function() RRT:UpdateRaidBuffFrame() end)
PVEFrame:HookScript("OnShow", function() RRT:UpdateRaidBuffFrame() end)
PVEFrame:HookScript("OnHide", function() RRT:UpdateRaidBuffFrame() end)
PVEFrameTab1:HookScript("OnClick", function() RRT:UpdateRaidBuffFrame() end)
PVEFrameTab2:HookScript("OnClick", function() RRT:UpdateRaidBuffFrame() end)
PVEFrameTab3:HookScript("OnClick", function() RRT:UpdateRaidBuffFrame() end)
FriendsFrameTab1:HookScript("OnClick", function() RRT:UpdateRaidBuffFrame() end)
FriendsFrameTab2:HookScript("OnClick", function() RRT:UpdateRaidBuffFrame() end)
FriendsFrameTab3:HookScript("OnClick", function() RRT:UpdateRaidBuffFrame() end)
FriendsFrameTab4:HookScript("OnClick", function() RRT:UpdateRaidBuffFrame() end)

function RRT:InviteFromReminder(str, init)
    local list = RRTDB.InviteList[str]
...
function RRT:ArrangeFromReminder(str)
