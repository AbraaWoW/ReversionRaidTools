...
function RRT_NS:RequestVersionNumber(type, name) -- type == "Addon" or "WA" or "Note"
    if type == "ReversionRaidTools" or (UnitIsGroupLeader("player") or UnitIsGroupAssistant("player") or RRT.Settings["Debug"]) then
...
function RRT_NS:GetVersionNumber(type, name, unit)
    ...
    if type == "ReversionRaidTools" then
        local ver = C_AddOns.GetAddOnMetadata("ReversionRaidTools", "Version") or "Not Installed"
        if ver ~= "Not Installed" then
            ver = C_AddOns.IsAddOnLoaded("ReversionRaidTools") and ver or "Not Enabled"
        end
