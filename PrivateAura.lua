...
function RRT:AddPASound(spellID, sound, unit)
    ...
    local soundPath = RRT.LSM:Fetch("sound", sound)
...
function RRT:ApplyDefaultPASounds(changed, mplus) -- only apply sound if changed == true, this happens when user changes the settings but not on login so we don't apply the sounds twice.
    local list = mplus and SoundListMPlus or SoundListRaid
    for spellID, sound in pairs(list) do
        local curSound = RRTDB.PASounds[spellID]
...
function RRT:SavePASound(spellID, sound)
    if (not spellID) then return end
    RRTDB.PASounds[spellID] = {sound = sound, edited = true}
...
function RRT:InitTextPA()
    if not self.PATextMoverFrame then
        self.PATextMoverFrame = CreateFrame("Frame", nil, self.RRTFrame)
        self.PATextMoverFrame:SetPoint(RRTDB.PATextSettings.Anchor, self.RRTFrame, RRTDB.PATextSettings.relativeTo, RRTDB.PATextSettings.xOffset, RRTDB.PATextSettings.yOffset)

        self.PATextMoverFrame.Text = self.PATextMoverFrame:CreateFontString(nil, "OVERLAY", "GameFontNormal")
        self.PATextMoverFrame.Text:SetFont(self.LSM:Fetch("font", RRTDB.Settings.GlobalFont), RRTDB.PATextSettings.Scale*20, "OUTLINE")
        ...
        self.PATextMoverFrame:SetScript("OnDragStop", function(Frame)
            self:StopFrameMove(Frame, RRTDB.PATextSettings)
        end)
    end
    if RRTDB.PATextSettings.enabled then
        ...
        self.PATextWarning:SetPoint("TOPLEFT", self.PATextMoverFrame, "TOPLEFT", 0, -0.8*height/RRTDB.PATextSettings.Scale)
        self.PATextWarning:SetPoint("BOTTOMRIGHT", self.PATextMoverFrame, "BOTTOMRIGHT", 0, -0.8*height/RRTDB.PATextSettings.Scale)
        self.PATextWarning:SetScale(RRTDB.PATextSettings.Scale)
...
function RRT:InitPA()
    ...
    local xDirection = (RRTDB.PASettings.GrowDirection == "RIGHT" and 1) or (RRTDB.PASettings.GrowDirection == "LEFT" and -1) or 0
    local yDirection = (RRTDB.PASettings.GrowDirection == "DOWN" and -1) or (RRTDB.PASettings.GrowDirection == "UP" and 1) or 0
    local borderSize = RRTDB.PASettings.HideBorder and -100 or RRTDB.PASettings.Width/16
    local scale = RRTDB.PASettings.StackScale or 4
    for auraIndex=1, 10 do
        ...
        if RRTDB.PASettings.enabled and RRTDB.PASettings.Limit >= auraIndex or auraIndex == 1 then
            ...
            if RRTDB.PASettings.HideTooltip then
                self.PAFrames[auraIndex]:SetSize(0.001, 0.001)
            else
                self.PAFrames[auraIndex]:SetSize(RRTDB.PASettings.Width, RRTDB.PASettings.Height)
            end
            self.PAAnchorFrames[auraIndex]:SetSize(RRTDB.PASettings.Width, RRTDB.PASettings.Height)
            ...
            self.PAAnchorFrames[auraIndex]:SetPoint(RRTDB.PASettings.Anchor, self.RRTFrame, RRTDB.PASettings.relativeTo,
            RRTDB.PASettings.xOffset+(auraIndex-1) * (RRTDB.PASettings.Width+RRTDB.PASettings.Spacing) * xDirection,
            RRTDB.PASettings.yOffset+(auraIndex-1) * (RRTDB.PASettings.Height+RRTDB.PASettings.Spacing) * yDirection)
            self.PAFrames[auraIndex]:SetPoint(RRTDB.PASettings.Anchor, self.RRTFrame, RRTDB.PASettings.relativeTo,
            RRTDB.PASettings.xOffset+(auraIndex-1) * (RRTDB.PASettings.Width+RRTDB.PASettings.Spacing) * xDirection,
            RRTDB.PASettings.yOffset+(auraIndex-1) * (RRTDB.PASettings.Height+RRTDB.PASettings.Spacing) * yDirection)
            if not RRTDB.PASettings.enabled then return end
            ...
            local privateAnchorArgs = {
                ...
                showCountdownNumbers = not RRTDB.PASettings.UpscaleDuration,
                iconInfo = {
                    iconAnchor = {
                        point = "CENTER",
                        relativeTo = frame,
                    ...
                    iconWidth = RRTDB.PASettings.Width,
                    iconHeight = RRTDB.PASettings.Height,
                },
            }
            ...
            if scale ~= 1 then
                ...
                if RRTDB.PASettings.UpscaleDuration then
...
function RRT:InitRaidPA(party, firstcall) -- still run this function if disabled to clean up old anchors
    ...
    local borderSize = RRTDB.PARaidSettings.HideBorder and -100 or RRTDB.PARaidSettings.Width/16
    local scale = RRTDB.PARaidSettings.StackScale or 1
    for i=1, party and 5 or 40 do
        ...
        if RRTDB.PARaidSettings.enabled and UnitExists(u) then
            ...
            if F then
                ...
                self.PARaidAnchorFrames[i]:SetSize(RRTDB.PARaidSettings.Width, RRTDB.PARaidSettings.Height)
                self.PAStackFrames[i]:SetScale(scale)
                self.PARaidFrames[i]:ClearAllPoints()
                self.PARaidAnchorFrames[i]:ClearAllPoints()
                self.PARaidAnchorFrames[i]:SetPoint(RRTDB.PARaidSettings.Anchor, F, RRTDB.PARaidSettings.relativeTo, RRTDB.PARaidSettings.xOffset, RRTDB.PARaidSettings.yOffset)
                self.PARaidFrames[i]:SetSize(0.001, 0.001)
                self.PARaidFrames[i]:SetPoint(RRTDB.PARaidSettings.Anchor, F, RRTDB.PARaidSettings.relativeTo, RRTDB.PARaidSettings.xOffset, RRTDB.PARaidSettings.yOffset)
                local xDirection = (RRTDB.PARaidSettings.GrowDirection == "RIGHT" and 1) or (RRTDB.PARaidSettings.GrowDirection == "LEFT" and -1) or 0
                local yDirection = (RRTDB.PARaidSettings.GrowDirection == "DOWN" and -1) or (RRTDB.PARaidSettings.GrowDirection == "UP" and 1) or 0
                local xRowDirection = (RRTDB.PARaidSettings.RowGrowDirection == "RIGHT" and 1) or (RRTDB.PARaidSettings.RowGrowDirection == "LEFT" and -1) or 0
                local yRowDirection = (RRTDB.PARaidSettings.RowGrowDirection == "DOWN" and -1) or (RRTDB.PARaidSettings.RowGrowDirection == "UP" and 1) or 0
                self.AddedPARaid[anchorID] = {}
                self.AddedPAStackRaid[anchorID] = {}
                for auraIndex = 1, 10 do
                    if auraIndex > RRTDB.PARaidSettings.Limit then break end
                    local row = math.ceil(auraIndex/RRTDB.PARaidSettings.PerRow)
                    local column = auraIndex - (row-1)*RRTDB.PARaidSettings.PerRow
                    local privateAnchorArgs = {
                        unitToken = u,
                        auraIndex = auraIndex,
                        parent = self.PARaidFrames[i],
                        showCountdownFrame = true,
                        showCountdownNumbers = not RRTDB.PARaidSettings.HideDurationText,
                        iconInfo = {
                            iconAnchor = {
                                point = RRTDB.PARaidSettings.Anchor,
                                relativeTo = self.PARaidFrames[i],
                                relativePoint = RRTDB.PARaidSettings.relativeTo,
                                offsetX = (column - 1) * (RRTDB.PARaidSettings.Width+RRTDB.PARaidSettings.Spacing) * xDirection + (row - 1) * (RRTDB.PARaidSettings.Width+RRTDB.PARaidSettings.Spacing) * xRowDirection,
                                offsetY = (column - 1) * (RRTDB.PARaidSettings.Height+RRTDB.PARaidSettings.Spacing) * yDirection + (row - 1) * (RRTDB.PARaidSettings.Height+RRTDB.PARaidSettings.Spacing) * yRowDirection,
                            },
                            borderScale = borderSize,
                            iconWidth = RRTDB.PARaidSettings.Width,
                            iconHeight = RRTDB.PARaidSettings.Height,
                        }
                    }
                        ...
                        local stackArgs = {
                            ...
                            iconInfo = {
                                iconAnchor = {
                                    ...
                                    relativePoint = "BOTTOMRIGHT",
                                    offsetX = 4/scale + ((column - 1) * (RRTDB.PARaidSettings.Width+RRTDB.PARaidSettings.Spacing) * xDirection)/scale + ((row - 1) * (RRTDB.PARaidSettings.Width+RRTDB.PARaidSettings.Spacing) * xRowDirection)/scale,
                                    offsetY = -4/scale + ((column - 1) * (RRTDB.PARaidSettings.Height+RRTDB.PARaidSettings.Spacing) * yDirection)/scale + ((row - 1) * (RRTDB.PARaidSettings.Height+RRTDB.PARaidSettings.Spacing) * yRowDirection)/scale,
                                },
                                borderScale = -100,
                                iconWidth = 0.001,
                                iconHeight = 0.001,
...
function RRT:InitTankPA()
    ...
    local xDirection = (RRTDB.PATankSettings.GrowDirection == "RIGHT" and 1) or (RRTDB.PATankSettings.GrowDirection == "LEFT" and -1) or 0
    local yDirection = (RRTDB.PATankSettings.GrowDirection == "DOWN" and -1) or (RRTDB.PATankSettings.GrowDirection == "UP" and 1) or 0

    local multiTankx = (RRTDB.PATankSettings.MultiTankGrowDirection == "RIGHT" and 1) or (RRTDB.PATankSettings.MultiTankGrowDirection == "LEFT" and -1) or 0
    local multiTanky = (RRTDB.PATankSettings.MultiTankGrowDirection == "DOWN" and -1) or (RRTDB.PATankSettings.MultiTankGrowDirection == "UP" and 1) or 0
    local units = {}
    ...
    self:RemoveTankPA()
    local borderSize = RRTDB.PATankSettings.HideBorder and -100 or RRTDB.PATankSettings.Width/16
    local scale = RRTDB.PATankSettings.StackScale or 4
    for i, unit in ipairs(units) do
        ...
        for auraIndex = 1, 10 do
            ...
            if RRTDB.PATankSettings.enabled and RRTDB.PATankSettings.Limit >= auraIndex then
                ...
                if RRTDB.PATankSettings.HideTooltip then
                    self.PATankFrames[i][auraIndex]:SetSize(0.001, 0.001)
                else
                    self.PATankFrames[i][auraIndex]:SetSize(RRTDB.PATankSettings.Width, RRTDB.PATankSettings.Height)
                end

                self.PATankAnchorFrames[i][auraIndex]:SetSize(RRTDB.PATankSettings.Width, RRTDB.PATankSettings.Height)
                ...
                self.PATankAnchorFrames[i][auraIndex]:SetPoint(RRTDB.PATankSettings.Anchor, self.RRTFrame, RRTDB.PATankSettings.relativeTo,
                RRTDB.PATankSettings.xOffset+(auraIndex-1) * (RRTDB.PATankSettings.Width+RRTDB.PATankSettings.Spacing) * xDirection + (i-1) * (RRTDB.PATankSettings.Width+RRTDB.PATankSettings.Spacing) * multiTankx,
                RRTDB.PATankSettings.yOffset+(auraIndex-1) * (RRTDB.PATankSettings.Height+RRTDB.PATankSettings.Spacing) * yDirection + (i-1) * (RRTDB.PATankSettings.Height+RRTDB.PATankSettings.Spacing) * multiTanky)
                self.PATankFrames[i][auraIndex]:SetPoint(RRTDB.PATankSettings.Anchor, self.RRTFrame, RRTDB.PATankSettings.relativeTo,
                RRTDB.PATankSettings.xOffset+(auraIndex-1) * (RRTDB.PATankSettings.Width+RRTDB.PATankSettings.Spacing) * xDirection + (i-1) * (RRTDB.PATankSettings.Width+RRTDB.PATankSettings.Spacing) * multiTankx,
                RRTDB.PATankSettings.yOffset+(auraIndex-1) * (RRTDB.PATankSettings.Height+RRTDB.PATankSettings.Spacing) * yDirection + (i-1) * (RRTDB.PATankSettings.Height+RRTDB.PATankSettings.Spacing) * multiTanky)
                ...
                local privateAnchorArgs = {
                    ...
                    showCountdownFrame = true,
                    showCountdownNumbers = not RRTDB.PATankSettings.UpscaleDuration,
                    iconInfo = {
                        iconAnchor = {
                            point = "CENTER",
                        ...
                        borderScale = borderSize,
                    iconWidth = RRTDB.PATankSettings.Width,
                    iconHeight = RRTDB.PATankSettings.Height,
                    }
                }
                ...
                if scale ~= 1 then
                    ...
                    if RRTDB.PATankSettings.UpscaleDurations then
                        durationArgs.durationAnchor = {
                            point = "CENTER",
                            relativeTo = self.PATankFrames[i][auraIndex],
                            relativePoint = "CENTER",
...
function RRT:UpdatePADisplay(Personal, Tank)
    if Personal then
...
function RRT:PreviewPA(Show)
    ...
    self.PAFrames[1]:SetSize((RRTDB.PASettings.Width), (RRTDB.PASettings.Height))
    self.PAFrames[1]:SetPoint(RRTDB.PASettings.Anchor, self.RRTFrame, RRTDB.PASettings.relativeTo, RRTDB.PASettings.xOffset, RRTDB.PASettings.yOffset)
    ...
    self.PAFrames[1]:SetScript("OnDragStop", function(Frame)
        self:StopFrameMove(Frame, RRTDB.PASettings)
    end)
    ...
    for i=1, 10 do
        ...
        if RRTDB.PASettings.Limit >= i then
            local xOffset = (RRTDB.PASettings.GrowDirection == "RIGHT" and (i-1)*(RRTDB.PASettings.Width+RRTDB.PASettings.Spacing)) or (RRTDB.PASettings.GrowDirection == "LEFT" and -(i-1)*(RRTDB.PASettings.Width+RRTDB.PASettings.Spacing)) or 0
            local yOffset = (RRTDB.PASettings.GrowDirection == "UP" and (i-1)*(RRTDB.PASettings.Height+RRTDB.PASettings.Spacing)) or (RRTDB.PASettings.GrowDirection == "DOWN" and -(i-1)*(RRTDB.PASettings.Height+RRTDB.PASettings.Spacing)) or 0
            self.PAPreviewIcons[i]:SetSize(RRTDB.PASettings.Width, RRTDB.PASettings.Height)
...
function RRT:PreviewTankPA(Show)
    ...
    if not self.PATankFrames[1] or not self.PATankFrames[1][1] then
        ...
        self.PATankFrames[1][1]:SetPoint(RRTDB.PATankSettings.Anchor, self.RRTFrame, RRTDB.PATankSettings.relativeTo, RRTDB.PATankSettings.xOffset, RRTDB.PATankSettings.yOffset)
    end
    if not Show then
        ...
        self:RemoveTankPA()
        if UnitGroupRolesAssigned("player") == "TANK" or RRTDB.Settings.Debug then
            self:InitTankPA()
        end
        return
    end
    self.PATankFrames[1][1]:SetSize((RRTDB.PATankSettings.Width), (RRTDB.PATankSettings.Height))
    self.PATankFrames[1][1]:SetPoint(RRTDB.PATankSettings.Anchor, self.RRTFrame, RRTDB.PATankSettings.relativeTo, RRTDB.PATankSettings.xOffset, RRTDB.PATankSettings.yOffset)
    ...
    self.PATankFrames[1][1]:SetScript("OnDragStop", function(Frame)
        self:StopFrameMove(Frame, RRTDB.PATankSettings)
    end)
    ...
    for i=1, 10 do
        ...
        if RRTDB.PATankSettings.Limit >= i then
            local xOffset = (RRTDB.PATankSettings.GrowDirection == "RIGHT" and (i-1)*(RRTDB.PATankSettings.Width+RRTDB.PATankSettings.Spacing)) or (RRTDB.PATankSettings.GrowDirection == "LEFT" and -(i-1)*(RRTDB.PATankSettings.Width+RRTDB.PATankSettings.Spacing)) or 0
            local yOffset = (RRTDB.PATankSettings.GrowDirection == "UP" and (i-1)*(RRTDB.PATankSettings.Height+RRTDB.PATankSettings.Spacing)) or (RRTDB.PATankSettings.GrowDirection == "DOWN" and -(i-1)*(RRTDB.PATankSettings.Height+RRTDB.PATankSettings.Spacing)) or 0
            self.PATankPreviewIcons[i]:SetSize(RRTDB.PATankSettings.Width, RRTDB.PATankSettings.Height)
...
function RRT:PreviewRaidPA(Show, Init)
    if not Show then
    ...
    self.PARaidPreviewFrame:SetSize(RRTDB.PARaidSettings.Width, RRTDB.PARaidSettings.Height)
    self.PARaidPreviewFrame:SetPoint(RRTDB.PARaidSettings.Anchor, MyFrame, RRTDB.PARaidSettings.relativeTo, RRTDB.PARaidSettings.xOffset, RRTDB.PARaidSettings.yOffset)
    ...
    local yRowDirection = (RRTDB.PARaidSettings.RowGrowDirection == "DOWN" and -1) or (RRTDB.PARaidSettings.RowGrowDirection == "UP" and 1) or 0
    for i=1, 10 do
        local row = math.ceil(i/RRTDB.PARaidSettings.PerRow)
        local column = i - (row-1)*RRTDB.PARaidSettings.PerRow
        if not self.PARaidPreviewIcons[i] then
            ...
            self.PARaidPreviewIcons[i].Text:SetFont(self.LSM:Fetch("font", RRTDB.Settings.GlobalFont), 16, "OUTLINE")
        ...
        if RRTDB.PARaidSettings.Limit >= i then
            local xOffset = (column - 1) * (RRTDB.PARaidSettings.Width+RRTDB.PARaidSettings.Spacing) * xDirection + (row - 1) * (RRTDB.PARaidSettings.Width+RRTDB.PARaidSettings.Spacing) * xRowDirection
            local yOffset = (column - 1) * (RRTDB.PARaidSettings.Height+RRTDB.PARaidSettings.Spacing) * yDirection + (row- 1) * (RRTDB.PARaidSettings.Height+RRTDB.PARaidSettings.Spacing) * yRowDirection
            self.PARaidPreviewIcons[i]:SetSize(RRTDB.PARaidSettings.Width, RRTDB.PARaidSettings.Height)
