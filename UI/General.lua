...
local function BuildExportStringUI()
    local popup = DF:CreateSimplePanel(RRTUI, 800, 800, "Export String", "RRTUIExportString", {
    ...
    for i, v in ipairs(buttonmapping) do
        ExportTable[v.name] = {data = RRTDB[v.name], enabled = true}
        local checkbox = DF:CreateSwitch(popup, function(self, fixedValue, value)
            ExportTable[v.name].enabled = value
            popup.test_string_text_box:SetText(RRT:CreateExportString(ExportTable) or "")
        end, true, 20, 20)
...
local function BuildImportStringUI()
    local popup = DF:CreateSimplePanel(RRTUI, 800, 800, "Import String", "RRTUIImportString", {
        DontRightClickClose = true
    })
    popup:SetPoint("CENTER", UIParent, "CENTER", 0, 0)
    popup:SetFrameLevel(100)
    ...
    popup.import_confirm_button = DF:CreateButton(popup, function()
        ...
        local ImportTable = RRT:ImportSettingsFromString(ImportString)
        ...
        RRT:ImportFromTable(ImportTable)
        popup:Hide()
    end, 280, 20, "Import")
    ...
    popup.test_string_text_box.editbox:SetScript("OnTextChanged", function(self)
        local ImportTable = RRT:ImportSettingsFromString(popup.test_string_text_box:GetText())
        if not ImportTable then return end
        SettingsTable = {}
        local num = 1
        for i, v in ipairs(buttonmapping) do
