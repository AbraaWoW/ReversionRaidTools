...
local function BuildPASoundEditUI()
    ...
    function RRT:RefreshPASoundEditUI()
    ...
    local function createLineFunc(self, index)
        ...
        line.sounddropdown:SetHook("OnOptionSelected", function(self, _, value)
            ...
            RRT:SavePASound(tonumber(line.spellID), newValue)
        ...
        line.deleteButton = DF:CreateButton(line, function()
            RRT:SavePASound(tonumber(line.spellID), nil)
            self:SetData(RRTDB.PASounds)
            self:MasterRefresh()
        end, 12, 12)
        ...
        local confirmButton = DF:CreateButton(popup, function()
            for spellID, info in pairs(RRTDB.PASounds) do
                if info and type(info) == "table" and info.sound then
                    RRT:AddPASound(spellID, nil)
                end
            end
            RRTDB.PASounds = {
                UseDefaultPASounds = RRTDB.PASounds.UseDefaultPASounds,
                UseDefaultMPlusPASounds = RRTDB.PASounds.UseDefaultMPlusPASounds
            }
            PASound_edit_scrollbox:MasterRefresh()
            popup:Hide()
        end, 100, 30, "Confirm")
