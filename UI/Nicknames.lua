...
local function BuildNicknameEditUI()
    ...
    local function PrepareData(data)
        local data = {}
        for player, nickname in pairs(RRTDB.NickNames) do
    ...
    local function createLineFunc(self, index)
        ...
        line.nicknameEntry = DF:CreateTextEntry(line, function(self, _, value)
            RRT:AddNickName(line.player, line.realm, string.sub(value, 1, 12))
        ...
        line.deleteButton = DF:CreateButton(line, function()
            RRT:AddNickName(line.player, line.realm, "")
            self:SetData(RRTDB.NickNames)
            self:MasterRefresh()
        end, 12, 12)
    ...
    local add_button = DF:CreateButton(nicknames_edit_frame, function()
        ...
        if player ~= "" and nickname ~= "" then
            ...
            RRT:AddNickName(player, realm, nickname)
    ...
    local sync_button = DF:CreateButton(nicknames_edit_frame, function() RRT:SyncNickNames() end, 225, 20, "Sync Nicknames")
    ...
    local function createImportPopup()
        ...
        popup.import_confirm_button = DF:CreateButton(popup, function()
            ...
            RRT:ImportNickNames(import_string)
