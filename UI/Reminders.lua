...
local function ImportReminderString(name, IsUpdate)
    local popup = DF:CreateSimplePanel(RRTUI, 800, 800, "Import Reminder String", "RRTUIReminderImport", {
    ...
    popup.test_string_text_box:SetText(name and RRTDB.Reminders[name] or "")
    popup.test_string_text_box:SetScript("OnMouseDown", function(self)
        self:SetFocus()
    end)
    popup.test_string_text_box.editbox:SetFont(RRT.LSM:Fetch("font", RRTDB.Settings.GlobalFont), 13, "OUTLINE")
    ...
    popup.import_confirm_button = DF:CreateButton(popup, function()
        ...
        if IsUpdate then
            RRT:ImportReminder(name, import_string, false, false, true)
        else
            RRT:ImportFullReminderString(import_string, false, false, name)
        end
        if IsUpdate and RRTDB.ActiveReminder then
            RRT:SetReminder(RRTDB.ActiveReminder) -- refresh active reminder
        end
...
local function ImportPersonalReminderString(name, IsUpdate)
    local popup = DF:CreateSimplePanel(RRTUI, 800, 800, "Import Personal Reminder String", "RRTUIPersonalReminderImport", {
        DontRightClickClose = true
    })
    popup:SetPoint("CENTER", UIParent, "CENTER", 0, 0)
    popup:SetFrameLevel(100)
    ...
    DF:ReskinSlider(popup.test_string_text_box.scroll)
    popup.test_string_text_box:SetFocus()
    popup.test_string_text_box:SetText(name and RRTDB.PersonalReminders[name] or "")
    popup.test_string_text_box:SetScript("OnMouseDown", function(self)
        self:SetFocus()
    end)
    popup.test_string_text_box.editbox:SetFont(RRT.LSM:Fetch("font", RRTDB.Settings.GlobalFont), 13, "OUTLINE")
    ...
    popup.import_confirm_button = DF:CreateButton(popup, function()
        local import_string = popup.test_string_text_box:GetText()
        if IsUpdate then
            RRT:ImportReminder(name, import_string, false, true, true)
        else
            RRT:ImportFullReminderString(import_string, true, false, name)
        end
        if IsUpdate and RRTDB.ActivePersonalReminder then
            RRT:SetReminder(RRTDB.ActivePersonalReminder, true) -- refresh active personal reminder
        end
        popup.test_string_text_box:SetText("")
...
local function BuildRemindersEditUI()
    ...
    local function MasterRefresh(self)
        local data = RRT:GetAllReminderNames()
        self:SetData(data)
        self:Refresh()
    end
    local function refresh(self, data, offset, totalLines)
        for i = 1, totalLines do
            ...
            if reminderData then
                ...
                if RRTDB.InviteList[reminderData.name] then
                ...
                if line.name == RRTDB.ActiveReminder then
    ...
    if RRTDB.ActiveReminder and RRTDB.ActiveReminder ~= "" then
        Active_Text.text = "Active Reminder: |cFFFFFFFF" .. RRTDB.ActiveReminder
    else
    ...
    local ClearButton = DF:CreateButton(reminders_edit_frame, function()
        ...
        RRT:Broadcast("RRT_REM_SHARE", "RAID", " ", nil, true)
    ...
    local function DeleteBossReminder(self, line, all)
        ...
        local confirmButton = DF:CreateButton(popup, function()
            if line and RRTDB.ActiveReminder and RRTDB.ActiveReminder == line.name then
                Active_Text.text = "Active Reminder: |cFFFFFFFFNone"
            end
            if all then
                if RRTDB.ActiveReminder then
                    Active_Text.text = "Active Reminder: |cFFFFFFFFNone"
                end
                for _, reminder in ipairs(RRT:GetAllReminderNames()) do
                    RRT:RemoveReminder(reminder.name)
                end
            else
                RRT:RemoveReminder(line.name)
            end
            self:SetData(RRT:GetAllReminderNames())
            self:MasterRefresh()
            popup:Hide()
        end, 100, 30, "Confirm")
    ...
    local function createLineFunc(self, index)
        ...
        local saveNewName = function(self)
            ...
            if RRTDB.Reminders[newname] then return end
            RRTDB.Reminders[newname] = RRTDB.Reminders[oldname]
            RRTDB.InviteList[newname] = RRTDB.InviteList[oldname]
            if RRTDB.ActiveReminder == oldname then
                Active_Text.text = "Active Reminder: |cFFFFFFFF" .. newname
                RRTDB.ActiveReminder = newname
            end
            RRTDB.Reminders[oldname] = nil
            RRTDB.InviteList[oldname] = nil
            line.name = newname
            parent:MasterRefresh()
        end
        ...
        line.LoadButton = DF:CreateButton(line, function()
            ...
            if name ~= "" then
                RRT:SetReminder(name)
                Active_Text.text = "Active Reminder: |cFFFFFFFF" .. name
                RRT:UpdateReminderFrame(true)
                RRT:Broadcast("RRT_REM_SHARE", "RAID", RRT.Reminder, nil, true)
                parent:MasterRefresh()
            end
        end, 40, 20, "Load")
        ...
        line.InviteButton = DF:CreateButton(line, function(self)
            RRT:InviteFromReminder(line.name, true)
        end, 40, 20, "Invite")
        ...
        line.ArrangeButton = DF:CreateButton(line, function(self)
            RRT:ArrangeFromReminder(line.name)
        end, 40, 20, "Arrange")
...
local function BuildPersonalRemindersEditUI()
    ...
    reminders_edit_frame:SetPoint("CENTER", UIParent, "CENTER", 0, 0)
    local function MasterRefresh(self)
        local data = RRT:GetAllReminderNames(true)
        self:SetData(data)
        self:Refresh()
    end
    local function refresh(self, data, offset, totalLines)
        for i = 1, totalLines do
            ...
            if reminderData then
                ...
                if line.name == RRTDB.ActivePersonalReminder then
                    line.nameTextEntry:SetBackdropBorderColor(0, 1, 0, 1)
                    line.nameTextEntry.BorderColorG = 1
                else
                    line.nameTextEntry:SetBackdropBorderColor(0, 0, 0, 1)
    ...
    Active_Text:SetPoint("BOTTOMLEFT", reminders_edit_frame, "BOTTOMLEFT", 5, 50)
    Active_Text:SetWidth(380)
    if RRTDB.ActivePersonalReminder and RRTDB.ActivePersonalReminder ~= "" then
        Active_Text.text = "Active Personal Reminder: |cFFFFFFFF" .. RRTDB.ActivePersonalReminder
    else
    ...
    local function DeleteBossReminder(self, line, all)
        ...
        local confirmButton = DF:CreateButton(popup, function()
            if RRTDB.ActivePersonalReminder and RRTDB.ActivePersonalReminder == line.name then
                Active_Text.text = "Active Personal Reminder: |cFFFFFFFFNone"
            end
            if all then
                if RRTDB.ActivePersonalReminder then
                    Active_Text.text = "Active Personal Reminder: |cFFFFFFFFNone"
                end
                for _, reminder in ipairs(RRT:GetAllReminderNames(true)) do
                    RRT:RemoveReminder(reminder.name, true)
                end
            else
                RRT:RemoveReminder(line.name, true)
            end
            self:SetData(RRT:GetAllReminderNames(true))
            self:MasterRefresh()
            popup:Hide()
        end, 100, 30, "Confirm")
        confirmButton:SetPoint("BOTTOMLEFT", popup, "BOTTOM", 5, 10)
    ...
    local function createLineFunc(self, index)
        ...
        local saveNewName = function(self)
            ...
            local newname = self:GetText()
            if oldname == newname then return end
            if RRTDB.PersonalReminders[newname] then return end
            RRTDB.PersonalReminders[newname] = RRTDB.PersonalReminders[oldname]
            if RRTDB.ActivePersonalReminder == oldname then
                Active_Text.text = "Active Personal Reminder: |cFFFFFFFF" .. newname
                RRTDB.ActivePersonalReminder = newname
            end
            RRTDB.PersonalReminders[oldname] = nil
            line.name = newname
            parent:MasterRefresh()
        end
        line.nameTextEntry:SetScript("OnEnterPressed", saveNewName)
