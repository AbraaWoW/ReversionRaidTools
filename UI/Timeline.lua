...
local function BuildTimelineTabUI(parent)
    ...
    parent.bossDisplayMode = RRT.BossDisplayModes.IMPORTANT_HEALER
        ...
        return {
            {
                ...
                onclick = function(_, _, value)
                    ...
                    RRT:RefreshEmbeddedTimeline(parent)
                end
            },
            {
    ...
    local function BuildReminderDropdownOptions()
        local options = {}
        local sharedList = RRT:GetAllReminderNames(false)
        ...
        local personalList = RRT:GetAllReminderNames(true)
    ...
    local bossDisplayDropdown = DF:CreateDropDown(parent, BuildBossDisplayModeOptions, RRT.BossDisplayModes.IMPORTANT_HEALER, 150)
    ...
    RRT:SetupTimelineHooks(timelineFrame)
    ...
    if timelineFrame.scaleSlider then
        timelineFrame.scaleSlider:HookScript("OnValueChanged", function()
            RRT:UpdateEmbeddedPhaseMarkers(parent)
        end)
    ...
    parent:SetScript("OnShow", function(self)
        RRT:RefreshEmbeddedTimeline(self)
    end)
