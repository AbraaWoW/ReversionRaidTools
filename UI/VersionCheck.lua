...
local function BuildVersionCheckUI(parent)
    ...
    local hide_version_response_button = DF:CreateSwitch(parent,
        function(self, _, value) RRTDB.Settings["VersionCheckRemoveResponse"] = value end,
        RRTDB.Settings["VersionCheckRemoveResponse"], 20, 20, nil, nil, nil, "VersionCheckResponseToggle", nil, nil, nil,
    ...
    component_name_entry:SetHook("OnEditFocusGained", function(self)
        component_name_entry.AddonAutoCompleteList = RRTDB.RRTUI.AutoComplete["Addon"] or {}
        ...
        for i = 1, totalLines do
            ...
            if thisData then
                ...
                line:SetScript("OnClick", function(self)
                    local message = ""
                    local now = GetTime()
                    if (RRT.VersionCheckData.lastclick[name] and now < RRT.VersionCheckData.lastclick[name] + 5) or (thisData.version == RRT.VersionCheckData.version and (not thisData.ignoreCheck)) or thisData.version == "No Response" then return end
                    RRT.VersionCheckData.lastclick[name] = now
                    if RRT.VersionCheckData.type == "Addon" then
                        if thisData.version == "Addon not enabled" then message = "Please enable the Addon: '"..RRT.VersionCheckData.name.."'"
                        elseif thisData.version == "Addon Missing" then message = "Please install the Addon: '"..RRT.VersionCheckData.name.."'"
                        else message = "Please update the Addon: '"..RRT.VersionCheckData.name.."'" end
                    elseif RRT.VersionCheckData.type == "Note" then
                    ...
                    RRT.VersionCheckData.lastclick[name] = GetTime()
    ...
    local function createLineFunc(self, index)
        ...
        name:SetFont(RRT.LSM:Fetch("font", RRTDB.Settings.GlobalFont), 12, "OUTLINE")
        ...
        version:SetFont(RRT.LSM:Fetch("font", RRTDB.Settings.GlobalFont), 12, "OUTLINE")
        ...
        ignorelist:SetFont(RRT.LSM:Fetch("font", RRTDB.Settings.GlobalFont), 12, "OUTLINE")
    ...
    local addData = function(self, data, url)
        ...
        if self.name_map[data.name] then
            if RRTDB.Settings["VersionCheckRemoveResponse"] and currentData[1] and currentData[1].version and data.version and data.version == currentData[1].version and data.version ~= "Addon Missing" and data.version ~= "Note Missing" and data.version ~= "Reminder Missing" and (not data.ignoreCheck) then
    ...
    version_check_button:SetScript("OnClick", function(self)
        ...
        local component_type = component_type_dropdown:GetValue()
        if text and text ~= ""  and component_type ~= "Note" and component_type ~= "Reminder" and not tContains(RRTDB.RRTUI.AutoComplete[component_type], text) then
            tinsert(RRTDB.RRTUI.AutoComplete[component_type], text)
        end

        if not text or text == "" and component_type ~= "Note" and component_type ~= "Reminder" then return end

        local now = GetTime()
        if RRT.LastVersionCheck and RRT.LastVersionCheck > now-2 then return end
        RRT.LastVersionCheck = now
        version_check_scrollbox:WipeData()
        local userData, url = RRT:RequestVersionNumber(component_type, text)
        if userData then
            RRT.VersionCheckData = { version = userData.version, type = component_type, name = text, url = url, lastclick = {} }
    ...
    local function build_version_check_presets_options()
        ...
        local t = {}
        for i = 1, #RRTDB.Settings["VersionCheckPresets"] do
            local v = RRTDB.Settings["VersionCheckPresets"][i]
            tinsert(t, {
                label = v[1],
                value = v[2],
                onclick = function(_, _, value)
    ...
    local function createPresetLineFunc(self, index)
        ...
        line.deleteButton = DF:CreateButton(line, function()
            tremove(RRTDB.Settings["VersionCheckPresets"], line.index)
            self:SetData(RRTDB.Settings["VersionCheckPresets"])
            self:Refresh()
    ...
    local version_presets_edit_scrollbox = DF:CreateScrollBox(version_presets_edit_frame,
        "$parentVersionPresetsEditScrollBox", refreshPresets, RRTDB.Settings["VersionCheckPresets"], 360,
    ...
    local add_button = DF:CreateButton(version_presets_edit_frame, function()
        ...
        tinsert(RRTDB.Settings["VersionCheckPresets"], { type .. ": " .. name, { type, name } })
        version_presets_edit_scrollbox:SetData(RRTDB.Settings["VersionCheckPresets"])
        version_presets_edit_scrollbox:Refresh()
        version_check_preset_dropdown:Refresh()
