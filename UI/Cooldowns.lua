...
local function build_spec_options()
    local t = {}
    local classSpecs = RRT.CLASS_SPECIALIZATION_MAP

    for className, specs in pairs(classSpecs) do
        for _, specId in pairs(specs) do
            tinsert(t, {
                label = RRT:SpecToName(specId),
                value = specId,
                className = className,
            })
        end
...
local function BuildCooldownsEditUI()
    ...
    local function PrepareData(data)
        local data = {}
        for specId, cooldowns in pairs(RRTDB.CooldownList) do
    ...
    local function refresh(self, data, offset, totalLines)
        for i = 1, totalLines do
            ...
            if cooldownData then
                ...
                line.specText.text = RRT:SpecToName(line.spec)
    ...
    local function createLineFunc(self, index)
        ...
        line.typeDropdown:SetHook("OnOptionSelected", function(self, _, value)
            ...
            if newType == "Spell" then
                RRT:RemoveTrackedCooldown(line.spec, line.id, string.lower(oldType))
                RRT:AddTrackedCooldown(line.spec, line.id, "spell", line.offset)
            else
                RRT:RemoveTrackedCooldown(line.spec, line.id, string.lower(oldType))
                RRT:AddTrackedCooldown(line.spec, line.id, "item", line.offset)
            end
        ...
        line.idTextEntry:SetScript("OnEnterPressed", function(self)
            ...
            if line.type == "Spell" then
                RRT:RemoveTrackedCooldown(line.spec, oldId, "spell")
                RRT:AddTrackedCooldown(line.spec, newId, "spell", line.offset)
            else
                RRT:RemoveTrackedCooldown(line.spec, oldId, "item")
                RRT:AddTrackedCooldown(line.spec, newId, "item", line.offset)
            end
        ...
        line.offsetSlider:SetHook("OnValueChanged", function(self, fixedValue, value)
            RRT:RemoveTrackedCooldown(line.spec, line.id, line.type)
            RRT:AddTrackedCooldown(line.spec, line.id, line.type, value)
            line.offset = value
            parent:MasterRefresh()
        end)
        ...
        line.deleteButton = DF:CreateButton(line, function()
            RRT:RemoveTrackedCooldown(line.spec, line.id, line.type)
            self:SetData(RRTDB.CooldownList)
            self:MasterRefresh()
        end, 12, 12)
    ...
    local add_button = DF:CreateButton(cooldowns_edit_frame, function()
        ...
        if spec and id ~= "" then
            RRT:AddTrackedCooldown(spec, id, type, offset)
