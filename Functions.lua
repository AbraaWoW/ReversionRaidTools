...
function RRT:IterateGroupMembers(reversed, forceParty)
...
function RRT:Restricted()
    return C_Secrets.ShouldAurasBeSecret()
end

function RRT:Print(...)
    if RRTDB.Settings["DebugLogs"] then
        if DevTool then
            local t = {...}
            local name = t[1]
...
function RRT:SortTable(t, reversed)
    table.sort(t,
        function(a, b)
...
function RRTAPI:Shorten(unit, num, specicon, AddonName, combined, roleicon) -- Returns color coded Name/Nickname
    ...
    if specicon then
        local specid = 0
        if unit then specid = RRT:GetSpecs(unit) or 0 end
    ...
    if classFile then -- basically "if unit found"
        ...
        name = num and RRT:Utf8Sub(RRTAPI:GetName(name, AddonName), 1, num) or RRTAPI:GetName(name, AddonName) -- shorten name before wrapping in color
...
function RRT:GetSpecs(unit)
    if unit then
        return RRT.specs[unit] or false -- return false if no information available for that unit so it goes to the next fallback
    else
        return RRT.specs -- if no unit is given then entire table is requested
    end
end


function RRT:GetNote() -- simply for note comparison now
...
function RRT:DifficultyCheck(num) -- check if current difficulty is a Normal/Heroic/Mythic raid and also allow checking if we are currently in an encounter
    local difficultyID = select(3, GetInstanceInfo()) or 0
    return ((difficultyID >= num and difficultyID <= 16 and difficultyID)) or (RRTDB.Settings["Debug"] and 16)
end

function RRT:GetHash(text)
    local counter = 1
...
local path = "Interface\\AddOns\\ReversionRaidTools\\Media\\Sounds\\"
function RRTAPI:TTS(sound, voice, overlap) -- RRTAPI:TTS("Bait Frontal")
    if RRTDB.Settings["TTS"] then
        ...
        if handle then
            ...
            local num = voice or RRTDB.Settings["TTSVoice"]
...
function RRT:SpecToName(specid)
...
function RRT:Utf8Sub(str, startChar, endChar)
...
function RRT:UnitAura(unit, spell) -- simplify aura checking for myself
...
RRT.Callbacks = RRT.Callbacks or LibStub("CallbackHandler-1.0"):New(RRT)

function RRT:FireCallback(event, ...)
    if RRTDB.ReminderSettings.UseTimelineReminders then
...
function RRTAPI:RegisterCallback(event, callback, owner)
    return RRT:RegisterCallback(event, callback, owner)
end

function RRTAPI:UnregisterCallback(event, callback, owner)
    return RRT:UnregisterCallback(event, callback, owner)
end

function RRTAPI:UnregisterAllCallbacks(owner)
    return RRT:UnregisterAllCallbacks(owner)
end

local Serialize = LibStub("AceSerializer-3.0")
local Compress = LibStub("LibDeflate")

function RRT:CreateExportString(SettingsTable) -- {"ReminderSettings", "PASettings", ...}
    local str = ""
    local ExportTable = {}
...
function RRT:ImportFromTable(ImportTable)
    local changed = false
...
function RRT:ImportSettingsFromString(string)
...
function RRT:StopFrameMove(F, SettingsTable)
