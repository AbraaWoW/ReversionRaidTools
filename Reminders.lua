...
function RRT:AddToReminder(info)
    ...
    if info.TTS == nil then
        info.TTS = (info.spellID and RRTDB.ReminderSettings.SpellTTS) or ((not info.spellID) and RRTDB.ReminderSettings.TextTTS)
    end
    if info.TTSTimer == nil then
        ...
        info.TTSTimer = info.dur or ((info.spellID and RRTDB.ReminderSettings.SpellTTSTimer) or RRTDB.ReminderSettings.TextTTSTimer)
    end
    if info.dur == nil then
        info.dur = info.spellID and RRTDB.ReminderSettings.SpellDuration or RRTDB.ReminderSettings.TextDuration
    end
    if info.countdown == nil then
        info.countdown = info.spellID and RRTDB.ReminderSettings.SpellCountdown or RRTDB.ReminderSettings.TextCountdown
    ...
    if (RRTDB.ReminderSettings.SpellName or RRTDB.ReminderSettings.SpellNameTTS) and info.spellID and not info.text then -- display spellname if text is empty, also make TTS that spellname
        local spell = C_Spell.GetSpellInfo(info.spellID)
        if spell then
            info.text = RRTDB.ReminderSettings.SpellName and spell.name or "" -- set text to SpellName
    ...
    if info.TTS and type(info.TTS) == "string" and ((RRTDB.ReminderSettings.AnnounceSpellDuration and info.spellID) or (RRTDB.ReminderSettings.AnnounceTextDuration and not info.spellID)) and not info.IsAlert then
    ...
    if RRTDB.ReminderSettings.PlayDefaultSound and (type(info.TTS) == "boolean" or not info.TTS) and (not info.sound) and (not info.IsAlert) then
        info.sound = RRTDB.ReminderSettings.DefaultSound
    end
...
function RRT:ProcessReminder()
    ...
    local pers = RRTDB.ReminderSettings.PersonalReminderFrame.enabled
    local shared = RRTDB.ReminderSettings.ReminderFrame.enabled
    ...
    if (RRTDB.ReminderSettings.enabled or RRTDB.ReminderSettings.UseTimelineReminders) and self.Reminder then str = self.Reminder end
    if RRTDB.ReminderSettings.MRTNote or RRTDB.ReminderSettings.UseTimelineReminders then
        local note = VMRT and VMRT.Note and VMRT.Note.Text1 or ""
        str = note and str ~= "" and str.."\n"..note or note or str
    end
    if RRTDB.ReminderSettings.PersNote or RRTDB.ReminderSettings.UseTimelineReminders then
    ...
    if str ~= "" then
        ...
        for line in str:gmatch('([^\n]*)\n') do
            ...
            if time and tag and (text or spellID) and encID and encID ~= 0 and not firstline then
                ...
                if (pers or shared) and (spellID or not RRTDB.ReminderSettings.OnlySpellReminders) then -- only insert this if it's a spell or user wants to see text-reminders as well
                    ...
                    local tagNames = ""
                    if not RRTDB.ReminderSettings.HidePlayerNames then
                    ...
                    displayLine = RRTDB.ReminderSettings.HidePlayerNames and displayLine:gsub("tag:([^;]+)", "") or displayLine:gsub("tag:([^;]+)", tagNames.." ")
                ...
                then
                    if not addedpersonalreminders[key] then
                        ...
                        if pers then
                            if (spellID or not RRTDB.ReminderSettings.OnlySpellReminders) then -- only insert this if it's a spell or user wants to see text-reminders as well
                ...
                if (not firstline) and (not line:find("invitelist:")) then
                    if RRTDB.Settings["GlobalNickNames"] and false then
                        local words = {}
...
function RRT:UpdateExistingFrames() -- called when user changes settings to not require a reload
    local parent = self.ReminderText or {}
    for i=1, #parent do
        local F = parent[i]
        if F and F:IsShown() then
            local s = RRTDB.ReminderSettings.TextSettings
            F.Text:SetFont(self.LSM:Fetch("font", s.Font), s.FontSize, "OUTLINE")
        end
    end
    self:ArrangeStates("Texts")
    self:MoveFrameSettings(self.TextMover, RRTDB.ReminderSettings.TextSettings, true)
    parent = self.ReminderIcon or {}
    for i=1, #parent do
        local F = parent[i]
        if F and F:IsShown() then
            local s = RRTDB.ReminderSettings.IconSettings
            F:SetSize(s.Width, s.Height)
            F.Icon:SetAllPoints(F)
            F.Border:SetAllPoints(F)
            local anchor = RRTDB.ReminderSettings.IconSettings.RightAlignedText and "RIGHT" or "LEFT"
            local relativePoint = RRTDB.ReminderSettings.IconSettings.RightAlignedText and "LEFT" or "RIGHT"
    ...
    self:MoveFrameSettings(self.IconMover, RRTDB.ReminderSettings.IconSettings)
    parent = self.UnitIcon or {}
    for i=1, #parent do
        local F = parent[i]
        if F and F:IsShown() then
            local s = RRTDB.ReminderSettings.UnitIconSettings
    ...
    for i=1, #parent do
        local F = parent[i]
        if F and F:IsShown() then
            local s = RRTDB.ReminderSettings.BarSettings
            F:SetSize(s.Width, s.Height)
    ...
    self:MoveFrameSettings(self.BarMover, RRTDB.ReminderSettings.BarSettings, false, true)
end

function RRT:ArrangeStates(Type)
    local F = (Type == "Texts" and self.ReminderText) or (Type == "Icons" and self.ReminderIcon) or (Type == "Bars" and self.ReminderBar)
    if not F then return end
    local s = (Type == "Texts" and RRTDB.ReminderSettings.TextSettings) or (Type == "Icons" and RRTDB.ReminderSettings.IconSettings) or (Type == "Bars" and RRTDB.ReminderSettings.BarSettings)
    local pos = {}
    for i=1, #F do
    ...
    for i, v in ipairs(pos) do
        ...
        if Type == "Texts" then
            v.Frame:SetPoint("BOTTOMLEFT", "RRTUIReminderTextMover", "BOTTOMLEFT", 0, 0 + yoffset)
            v.Frame:SetPoint("TOPRIGHT", "RRTUIReminderTextMover", "TOPRIGHT", 0, 0 + yoffset)
        elseif Type == "Icons" then
            v.Frame:SetPoint("BOTTOMLEFT", "RRTUIReminderIconMover", "BOTTOMLEFT", 0 + xoffset, 0 + yoffset)
            v.Frame:SetPoint("TOPRIGHT", "RRTUIReminderIconMover", "TOPRIGHT", 0 + xoffset, 0 + yoffset)
        elseif Type == "Bars" then
            v.Frame:SetPoint("BOTTOMLEFT", "RRTUIReminderBarMover", "BOTTOMLEFT", 0, 0 + yoffset)
            v.Frame:SetPoint("TOPRIGHT", "RRTUIReminderBarMover", "TOPRIGHT", 0, 0 + yoffset)
        else
            print("RELOE PLS FIX (Reminder anchoring issue @ RRT:ArrangeStates)")
        end
    end
end

function RRT:SetProperties(F, info, skipsound, s)
    ...
    F:SetScript("OnHide", function()
        ...
        if F.Swipe and RRTDB.ReminderSettings.IconSettings.Glow > 0 then
            self:HideGlows(nil, nil, F)
        end
        RRT:ArrangeStates(F.Type)
    end)
    F.info = info
    ...
    if F.Swipe then
        if info.skipdur then
            ...
            if RRTDB.ReminderSettings.HideTimerText then
                F.TimerText:Hide()
            else
...
function RRT:CreateText(info)
    ...
    for i=1, #self.ReminderText+1 do
        ...
        if not self.ReminderText[i] then
            self.ReminderText[i] = CreateFrame("Frame", 'RRTUIReminderText' .. i, UIParent, "BackdropTemplate")
            local offset = s.GrowDirection == "Up" and (i-1) * s.FontSize or -(i-1) * s.FontSize
            self.ReminderText[i]:SetPoint("BOTTOMLEFT", "RRTUIReminderTextMover", "BOTTOMLEFT", 0, 0 + offset)
            self.ReminderText[i]:SetPoint("TOPRIGHT", "RRTUIReminderTextMover", "TOPRIGHT", 0, 0 + offset)
...
function RRT:CreateIcon(info)
    ...
    for i=1, #self.ReminderIcon+1 do
        ...
        if not self.ReminderIcon[i] then
            self.ReminderIcon[i] = CreateFrame("Frame", 'RRTUIReminderIcon' .. i, UIParent, "BackdropTemplate")
            ...
            self.ReminderIcon[i]:SetPoint("BOTTOMLEFT", "RRTUIReminderIconMover", "BOTTOMLEFT", 0 + xoffset, 0 + yoffset)
            self.ReminderIcon[i]:SetPoint("TOPRIGHT", "RRTUIReminderIconMover", "TOPRIGHT", 0 + xoffset, 0 + yoffset)
...
function RRT:CreateUnitFrameIcon(info, name)
...
function RRT:CreateBar(info)
    ...
    for i=1, #self.ReminderBar+1 do
        ...
        if not self.ReminderBar[i] then
            self.ReminderBar[i] = CreateFrame("StatusBar", 'RRTUIReminderBar' .. i, UIParent, "BackdropTemplate")
            ...
            self.ReminderBar[i]:SetPoint("BOTTOMLEFT", "RRTUIReminderBarMover", "BOTTOMLEFT", 0, 0 + offset)
            self.ReminderBar[i]:SetPoint("TOPRIGHT", "RRTUIReminderBarMover", "TOPRIGHT", 0, 0 + offset)
...
function RRT:DisplayReminder(info)
    local now = GetTime()
    ...
    if info.spellID and rem <= (0-RRTDB.ReminderSettings.Sticky) or ((info.notsticky or not info.spellID) and rem <= 0) then
        return
    end
    local remString
    if rem < 3 then
    ...
    if info.spellID then -- display icon if we have a spellID
        if (RRTDB.ReminderSettings.Bars or info.BarOverwrite) and not info.IconOverwrite then
...
function RRT:UpdateReminderDisplay(info, F, skipsound)
    local rem = info.dur - (GetTime() - info.startTime)
    local SoundTimer = info.TTSTimer or (info.spellID and RRTDB.ReminderSettings.SpellTTSTimer or RRTDB.ReminderSettings.TextTTSTimer)
    ...
    if info.spellID and type(info.spellID) == "number" then
        if F:GetObjectType() == "StatusBar" then
            ...
            if F.Swipe and RRTDB.ReminderSettings.IconSettings.Glow > 0 and rem <= RRTDB.ReminderSettings.IconSettings.Glow and not self.GlowStarted["ph"..info.phase.."id"..info.id] then
...
function RRT:PlayReminderSound(info, default)
...
function RRT:StartReminders(phase, testrun)
    ...
    if testrun then
        ...
        for encID, encData in pairs(self.ProcessedReminder) do
            for i, info in ipairs(encData[phase] or {}) do
                if info.IsAlert or not RRTDB.ReminderSettings.UseTimelineReminders then
...
function RRT:DelayAllReminders(delay)
...
function RRT:HideAllReminders(FullReset)
    self.PlayedSound = {}
...
function RRT:GetAllReminderNames(personal)
    local list = {}
    local tocheck = personal and RRTDB.PersonalReminders or RRTDB.Reminders
...
function RRT:SetReminder(name, personal, skipupdate)
    if personal then
        if name and RRTDB.PersonalReminders[name] then
            self.PersonalReminder = RRTDB.PersonalReminders[name]
            RRTDB.ActivePersonalReminder = name
    ...
    elseif name and RRTDB.Reminders[name] then
        self.Reminder = RRTDB.Reminders[name]
        RRTDB.ActiveReminder = name
        self:ProcessReminder()
        if not skipupdate then self:UpdateReminderFrame(true) end
    else
        self.Reminder = ""
    ...
    if not skipupdate then self:FireCallback("RRT_REMINDER_CHANGED", self.PersonalReminder, self.Reminder) end
end

function RRT:RemoveReminder(name, personal)
    if personal then
        if name and RRTDB.PersonalReminders[name] then
            RRTDB.PersonalReminders[name] = nil
            if RRTDB.ActivePersonalReminder == name then
                self:SetReminder(nil, true)
            end
        end
    elseif name and RRTDB.Reminders[name] then
        RRTDB.Reminders[name] = nil
        RRTDB.InviteList[name] = nil
        if RRTDB.ActiveReminder == name then
            self:SetReminder(nil, false)
        end
    end
end

function RRT:ImportFullReminderString(str, personal, IsUpdate, name)
    local name = ""
    local values = ""
    local diff = ""
    if not str:match('\n$') then
...
function RRT:ImportReminder(name, values, activate, personal, IsUpdate, diff)
    if not name then name = "Default Reminder" end
    local newname = diff and name.." - "..diff or name
    if personal then
        if RRTDB.PersonalReminders[newname] and not IsUpdate then -- if name already exists we add a 2 at the end
            self:ImportReminder(name.." 2", values, activate, personal, IsUpdate, diff)
            return
        end
        RRTDB.PersonalReminders[newname] = values
        if activate then
            self:SetReminder(newname, true)
        end
        return
    end
    if RRTDB.Reminders[newname] and not IsUpdate then -- if name already exists we add a 2 at the end
        self:ImportReminder(name.." 2", values, activate, personal, IsUpdate, diff)
        return
    end
    RRTDB.Reminders[newname] = values
    RRTDB.InviteList[newname] = self:InviteListFromReminder(values)
    if activate then
        self:SetReminder(newname)
    end
end

function RRT:InviteListFromReminder(str)
    local list = {}
    local found = false
    for line in str:gmatch('[^\r\n]+') do
...
function RRT:GlowFrame(unit, id, F)
    if F then
        local s = RRTDB.ReminderSettings.GlowSettings
...
function RRT:HideGlows(units, id, F)
    if F then
...
function RRT:CreateMoveFrames()
    self:CreateReminderMoverFrame("IconMover", RRTDB.ReminderSettings.IconSettings, "IconSettings")
    self:CreateReminderMoverFrame("BarMover", RRTDB.ReminderSettings.BarSettings, "BarSettings")
    self:CreateReminderMoverFrame("TextMover", RRTDB.ReminderSettings.TextSettings, "TextSettings", true)
    self:CreateNoteMoverFrame("ReminderFrame", RRTDB.ReminderSettings.ReminderFrame, true, false, false)
    self:CreateNoteMoverFrame("PersonalReminderFrame", RRTDB.ReminderSettings.PersonalReminderFrame, false, true, false)
    self:CreateNoteMoverFrame("ExtraReminderFrame", RRTDB.ReminderSettings.ExtraReminderFrame, false, false, true)
end

function RRT:CreateReminderMoverFrame(Name, SettingsTable, SettingsName, IsText)
    if not self[Name] then
        self[Name] = CreateFrame("Frame", 'RRTUIReminder'..Name, UIParent, "BackdropTemplate")
        if IsText then
...
function RRT:CreateNoteMoverFrame(Name, SettingsTable, Shared, Personal, Extra)
...
function RRT:MoveFrameSettings(F, s, IsText)
...
function RRT:MoveFrameInit(F, s, ReminderColor)
    if F then
        F.Border = CreateFrame("Frame", nil, F, "BackdropTemplate")
        local x = s == "BarSettings" and -6-RRTDB.ReminderSettings[s].Height or -6 -- extra offset for bars to account for the icon
        ...
        F:SetScript("OnDragStop", function(Frame)
            local settingsTable = s == "Generic" and RRTDB.Settings.GenericDisplay or RRTDB.ReminderSettings[s]
...
function RRTAPI:DebugNextPhase(num)
    if not RRTDB.Settings["Debug"] then return end
    for i=1, num do
        RRT:EventHandler("ENCOUNTER_TIMELINE_EVENT_ADDED")
    end
end

function RRTAPI:DebugEncounter(EncounterID)
    if not RRTDB.Settings["Debug"] then return end
    RRT.ProcessedReminder = nil
    RRT.Assignments = RRTDB.AssignmentSettings
    RRT:EventHandler("ENCOUNTER_START", true, true, EncounterID)
end
...
function RRTAPI:DebugTimeline(e, dur)
    if not RRTDB.Settings["Debug"] then return end
    RRT:EventHandler(e, true, true, {duration = dur})
end

function RRT:CreateDefaultAlert(text, Type, spellID, dur, phase, encID)
    ...
    local info =
    {
        ...
        text = text,
        TTS = (Type == "Text" and RRTDB.ReminderSettings.TextTTS and text) or (Type ~= "Text" and RRTDB.ReminderSettings.SpellTTS and text), -- use the user's settings
        notsticky = true,
...
function RRT:UpdateReminderFrame(all, shared, personal, extra)
    if all or shared then
        self:MoveFrameSettings(self.ReminderFrameMover, RRTDB.ReminderSettings.ReminderFrame)
        if not self.ReminderFrame then
            self:CreateNoteFrame("ReminderFrame", RRTDB.ReminderSettings.ReminderFrame)
        end
        local text = RRTDB.ReminderSettings.TextInSharedNote and self.DisplayedExtraReminder..self.DisplayedReminder or self.DisplayedReminder
        self:UpdateNoteFrame("ReminderFrame", RRTDB.ReminderSettings.ReminderFrame, text)
    end
    if all or personal then
        self:MoveFrameSettings(self.PersonalReminderFrameMover, RRTDB.ReminderSettings.PersonalReminderFrame)
        if not self.PersonalReminderFrame then
            self:CreateNoteFrame("PersonalReminderFrame", RRTDB.ReminderSettings.PersonalReminderFrame)
        end
        local text = RRTDB.ReminderSettings.TextInPersonalNote and self.DisplayedExtraReminder..self.DisplayedPersonalReminder or self.DisplayedPersonalReminder
        self:UpdateNoteFrame("PersonalReminderFrame", RRTDB.ReminderSettings.PersonalReminderFrame, text)
    end
    if all or extra then
        self:MoveFrameSettings(self.ExtraReminderFrameMover, RRTDB.ReminderSettings.ExtraReminderFrame)
        if not self.ExtraReminderFrame then
            self:CreateNoteFrame("ExtraReminderFrame", RRTDB.ReminderSettings.ExtraReminderFrame)
        end
        local text = self.DisplayedExtraReminder
        self:UpdateNoteFrame("ExtraReminderFrame", RRTDB.ReminderSettings.ExtraReminderFrame, text)
    end
end

function RRTAPI:GetReminderString()
    return RRT.PersonalReminder, RRT.Reminder
end

function RRT:CreateNoteFrame(Name, SettingsTable)
...
function RRT:UpdateNoteFrame(Name, SettingsTable, text)
...
function RRT:FlashFrameBackground(F, SettingsTable)
...
function RRT:FlashNoteBackgrounds()
    self:FlashFrameBackground(RRT.ReminderFrameMover, RRTDB.ReminderSettings.ReminderFrame)
    self:FlashFrameBackground(RRT.PersonalReminderFrameMover, RRTDB.ReminderSettings.PersonalReminderFrame)
    self:FlashFrameBackground(RRT.ExtraReminderFrameMover, RRTDB.ReminderSettings.ExtraReminderFrame)
end
