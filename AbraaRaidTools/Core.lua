...
-------------------------------------------------------------------------------
-- Module namespace (shared across AbraaRaidTools files via addon table)
-------------------------------------------------------------------------------
...
function ST:Print(msg)
    print("|cFF33FF99[Abraa Raid Tools]|r " .. tostring(msg))
end
...
function ST:PrintWelcome()
    ST:Print("Abraa Raid Tools — Menu: /art");
    if (IsAddonOutdated()) then
        print("|cFFFF9900[Abraa Raid Tools] WARNING:|r Addon may be outdated for this game version. Some features might not work correctly.");
    end
end
...
local function getDB()
    if (not _G.AbraaRaidToolsDB) then
        if (type(_G.ReversionRaidToolsDB) == "table") then
            _G.AbraaRaidToolsDB = _G.ReversionRaidToolsDB;
        else
            _G.AbraaRaidToolsDB = {};
        end
    end
    local db = _G.AbraaRaidToolsDB;
...
-- Bootstrap is handled by ReversionRaidTools EventHandler.lua
-- ST:Init() is called on ADDON_LOADED, ST:Enable() on PLAYER_LOGIN
