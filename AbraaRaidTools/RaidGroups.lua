...
local function BuildExportString(slots)
    ...
    return "ART_RG1:" .. table.concat(parts, "\t");
end
...
local function ParseImportedSlots(text)
    ...
    if (string.sub(raw, 1, 8) == "ART_RG1:") then
    ...
    return nil, "Unsupported text format. Use ART_RG1, text formats (20/5/40 lines), or 8 lines by group.";
end
...
function ST:BuildRaidGroupsSection(parent, yOff, FONT, PADDING, ROW_HEIGHT,
    ...
    Track(CreateActionButton(parent, profX, ioTopY - 94, "Export", 80, function()
        ...
        ST:Print("Raid groups exported (ART_RG1). You can copy the text.");
    end));
