...
function RRT:GemCheck(slot, itemString)
    local gemsMissing = 0
...
function RRT:EnchantCheck(slot, itemString)
...
function RRT:GearCheck()
    ...
    for slot = 1, #SlotName do
        ...
        if itemString then
            if RRTDB.ReadyCheckSettings.CraftedCheck and string.find(itemString, "8960") then
                crafted = crafted+1
            end
            if RRTDB.ReadyCheckSettings.EnchantCheck and UnitLevel("player") >= 90 and self:EnchantCheck(slot,itemString) then
                table.insert(missing, "Missing Enchant on: |cFF00FF00"..SlotName[slot].."|r")
            end
            if RRTDB.ReadyCheckSettings.GemCheck and UnitLevel("player") >= 90 and self:GemCheck(slot, itemString) then
                table.insert(missing, "Missing Gem in: |cFF00FF00"..SlotName[slot].."|r")
            end
            if RRTDB.ReadyCheckSettings.ItemLevelCheck and slot ~= 4 and select(4, C_Item.GetItemInfo(itemString)) < ilvl then
                table.insert(missing, "Low Itemlvl equipped on: |cFF00FF00"..SlotName[slot].."|r")
            end
            if RRTDB.ReadyCheckSettings.RepairCheck and not repair then
                local min, max = GetInventoryItemDurability(slot)
                if min and min/max <= 0.2 then
                    repair = true
                end
            end
            if RRTDB.ReadyCheckSettings.TierCheck then
                if self:TierCheck(slot) then
                    tier = tier+1
                end
            end
            -- Cloak is always considered Cloth, also don't even need to check for cloth wearers
            if MyArmorType ~= 1 and RRTDB.ReadyCheckSettings.MissingItemCheck and slot ~= 4 and slot ~= 15 then
        ...
        elseif RRTDB.ReadyCheckSettings.MissingItemCheck and slot ~= 4 then
            if slot == 17 then
    ...
    if RRTDB.ReadyCheckSettings.GatewayShardCheck then
        local Gateway = self:GatewayControlCheck()
        if Gateway then table.insert(missing, Gateway) end
    end
    if UnitLevel("player") >= 90 and RRTDB.ReadyCheckSettings.GemCheck and not self.MainstatGem then
        table.insert(missing, "Missing |cFF00FF00Mainstat Gem|r")
    end
    if repair then
        table.insert(missing, "Item needs |cFF00FF00Repair|r")
    end
    if RRTDB.ReadyCheckSettings.CraftedCheck and crafted < 2 then
        table.insert(missing, "Missing |cFF00FF00Embellishment|r")
    end
    if RRTDB.ReadyCheckSettings.TierCheck and tier < 4 then
        if tier < 2 then
...
function RRT:GatewayControlCheck()
...
function RRT:CheckGateWayKeybind(Slot)
